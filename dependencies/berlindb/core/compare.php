...
class Compare extends Meta
{
    ...
    public function get_sql_for_clause(&$clause, $parent_query, $clause_key = '')
    {
        ...
        if (!\in_array($clause['compare'], array('=', '!=', '>', '>=', '<', '<=', 'LIKE', 'NOT LIKE', 'IN', 'NOT IN', 'BETWEEN', 'NOT BETWEEN', 'EXISTS', 'NOT EXISTS', 'REGEXP', 'NOT REGEXP', 'RLIKE'), \true)) {
            $clause['compare'] = '=';
        }
        ...
        if (\array_key_exists('key', $clause) && \array_key_exists('value', $clause)) {
            ...
            $value = $clause['value'];
            if (\in_array($compare, array('IN', 'NOT IN', 'BETWEEN', 'NOT BETWEEN'), \true)) {
                if (!\is_array($value)) {
