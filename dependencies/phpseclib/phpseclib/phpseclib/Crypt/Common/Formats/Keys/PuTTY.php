...
abstract class PuTTY
{
    ...
    private static function generateSymmetricKey($password, $length)
    {
        ...
        while (\strlen($symkey) < $length) {
            ...
            $symkey .= \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Hex::decode(\sha1($temp));
        }
    ...
    public static function load($key, $password)
    {
        ...
        if (!\in_array($type, static::$types)) {
            ...
            throw new \phpseclib3\Exception\UnsupportedAlgorithmException($error . 'This is an unsupported ' . $type . ' key');
        }
        ...
        $public = \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::decode(\implode('', \array_map('trim', \array_slice($key, 4, $publicLength))));
        $source = \phpseclib3\Common\Functions\Strings::packSSH2('ssss', $type, $encryption, $components['comment'], $public);
        \extract(\unpack('Nlength', \phpseclib3\Common\Functions\Strings::shift($public, 4)));
        $newtype = \phpseclib3\Common\Functions\Strings::shift($public, $length);
        if ($newtype != $type) {
        ...
        $private = \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::decode(\implode('', \array_map('trim', \array_slice($key, $publicLength + 5, $privateLength))));
        switch ($encryption) {
            case 'aes256-cbc':
                $symkey = self::generateSymmetricKey($password, 32);
                $crypto = new \phpseclib3\Crypt\AES('cbc');
        }
        ...
        $source .= \phpseclib3\Common\Functions\Strings::packSSH2('s', $private);
        ...
        $hmac = \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Hex::decode($hmac);
    ...
    protected static function wrapPrivateKey($public, $private, $type, $password, array $options = [])
    {
        ...
        $key .= $encryption;
        $key .= "\r\nComment: " . $comment . "\r\n";
        $public = \phpseclib3\Common\Functions\Strings::packSSH2('s', $type) . $public;
        $source = \phpseclib3\Common\Functions\Strings::packSSH2('ssss', $type, $encryption, $comment, $public);
        $public = \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::encode($public);
        ...
        if (empty($password) && !\is_string($password)) {
            ...
            $private .= \phpseclib3\Crypt\Random::string(16 - (\strlen($private) & 15));
            $source .= \phpseclib3\Common\Functions\Strings::packSSH2('s', $private);
            $crypto = new \phpseclib3\Crypt\AES('cbc');
        ...
        $private = \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::encode($private);
        $key .= 'Private-Lines: ' . (\strlen($private) + 63 >> 6) . "\r\n";
        $key .= \chunk_split($private, 64);
        $hash = new \phpseclib3\Crypt\Hash('sha1');
        $hash->setKey(\sha1($hashkey, \true));
        $key .= 'Private-MAC: ' . \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Hex::encode($hash->hash($source)) . "\r\n";
        return $key;
    }
    /**
    ...
    protected static function wrapPublicKey($key, $type)
    {
        ...
        $key = "---- BEGIN SSH2 PUBLIC KEY ----\r\n" . 'Comment: "' . \str_replace(['\\', '"'], ['\\\\', '\\"'], self::$comment) . "\"\r\n" . \chunk_split(\WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::encode($key), 64) . '---- END SSH2 PUBLIC KEY ----';
        return $key;
    }
}
