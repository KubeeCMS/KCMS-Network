...
abstract class MontgomeryPrivate
{
    ...
    public static function load($key, $password = '')
    {
        ...
        $components['dA'] = $components['curve']->convertInteger(new \phpseclib3\Math\BigInteger($key, 256));
     ...
     * @return string
     */
    public static function savePrivateKey(\phpseclib3\Math\Common\FiniteField\Integer $privateKey, \phpseclib3\Crypt\EC\BaseCurves\Montgomery $curve, array $publicKey, $password = '')
    {
        if (!empty($password) && \is_string($password)) {
            throw new \phpseclib3\Exception\UnsupportedFormatException('MontgomeryPrivate private keys do not support encryption');
        }
