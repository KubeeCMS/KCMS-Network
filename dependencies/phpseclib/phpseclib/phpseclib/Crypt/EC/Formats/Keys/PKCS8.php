...
abstract class PKCS8 extends \phpseclib3\Crypt\Common\Formats\Keys\PKCS8
{
    ...
    public static function load($key, $password = '')
    {
        ...
        $params = \phpseclib3\File\ASN1::asn1map($decoded[0], \phpseclib3\File\ASN1\Maps\ECParameters::MAP);
        if (!$params) {
            throw new \RuntimeException('WP_Ultimo\\Dependencies\\Unable to decode the parameters using Maps\\ECParameters');
        }
        $components = [];
        ...
        $decoded = \phpseclib3\File\ASN1::decodeBER($key['privateKey']);
    ...
    private static function loadEdDSA(array $key)
    {
        ...
        if (isset($key['privateKey'])) {
            $components['curve'] = $key['privateKeyAlgorithm']['algorithm'] == 'id-Ed25519' ? new \phpseclib3\Crypt\EC\Curves\Ed25519() : new \phpseclib3\Crypt\EC\Curves\Ed448();
            // 0x04 == octet string
        ...
        if (isset($key['publicKey'])) {
            if (!isset($components['curve'])) {
                $components['curve'] = $key['publicKeyAlgorithm']['algorithm'] == 'id-Ed25519' ? new \phpseclib3\Crypt\EC\Curves\Ed25519() : new \phpseclib3\Crypt\EC\Curves\Ed448();
            }
            $components['QA'] = self::extractPoint($key['publicKey'], $components['curve']);
        }
    ...
    {
        self::initialize_static_variables();
        if ($curve instanceof \phpseclib3\Crypt\EC\BaseCurves\Montgomery) {
            throw new \phpseclib3\Exception\UnsupportedCurveException('Montgomery Curves are not supported');
        }
        if ($curve instanceof \phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
            return self::wrapPublicKey($curve->encodePoint($publicKey), null, $curve instanceof \phpseclib3\Crypt\EC\Curves\Ed25519 ? 'id-Ed25519' : 'id-Ed448');
        }
        $params = new \phpseclib3\File\ASN1\Element(self::encodeParameters($curve, \false, $options));
    ...
    public static function savePrivateKey(\phpseclib3\Math\Common\FiniteField\Integer $privateKey, \phpseclib3\Crypt\EC\BaseCurves\Base $curve, array $publicKey, $password = '', array $options = [])
    {
        ...
        if ($curve instanceof \phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
            return self::wrapPrivateKey("\4 " . $privateKey->secret, [], null, $password, $curve instanceof \phpseclib3\Crypt\EC\Curves\Ed25519 ? 'id-Ed25519' : 'id-Ed448', "\0" . $curve->encodePoint($publicKey));
        }
