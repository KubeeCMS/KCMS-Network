...
abstract class PuTTY extends \phpseclib3\Crypt\Common\Formats\Keys\PuTTY
{
    ...
    public static function load($key, $password = '')
    {
        ...
        $temp = \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::encode(\phpseclib3\Common\Functions\Strings::packSSH2('s', $components['type']) . $components['public']);
        $components = \phpseclib3\Crypt\EC\Formats\Keys\OpenSSH::load($components['type'] . ' ' . $temp . ' ' . $components['comment']);
        if ($components['curve'] instanceof \phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
            if (\phpseclib3\Common\Functions\Strings::shift($private, 4) != "\0\0\0 ") {
                throw new \RuntimeException('Length of ssh-ed25519 key should be 32');
            }
            $components['dA'] = $components['curve']->extractSecret($private);
        } else {
            list($temp) = \phpseclib3\Common\Functions\Strings::unpackSSH2('i', $private);
     ...
     * @return string
     */
    public static function savePrivateKey(\phpseclib3\Math\Common\FiniteField\Integer $privateKey, \phpseclib3\Crypt\EC\BaseCurves\Base $curve, array $publicKey, $password = \false, array $options = [])
    {
        self::initialize_static_variables();
        $public = \explode(' ', \phpseclib3\Crypt\EC\Formats\Keys\OpenSSH::savePublicKey($curve, $publicKey));
        $name = $public[0];
        $public = \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::decode($public[1]);
        list(, $length) = \unpack('N', \phpseclib3\Common\Functions\Strings::shift($public, 4));
        \phpseclib3\Common\Functions\Strings::shift($public, $length);
        ...
        $private = $curve instanceof \phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards ? \phpseclib3\Common\Functions\Strings::packSSH2('s', $privateKey->secret) : \phpseclib3\Common\Functions\Strings::packSSH2('s', $private);
