 ...
 * @access  public
 */
class PublicKey extends \phpseclib3\Crypt\EC implements \phpseclib3\Crypt\Common\PublicKey
{
    ...
    public function verify($message, $signature)
    {
        ...
        if ($this->curve instanceof \phpseclib3\Crypt\EC\BaseCurves\TwistedEdwards) {
            if ($shortFormat == 'SSH2') {
                list(, $signature) = \phpseclib3\Common\Functions\Strings::unpackSSH2('ss', $signature);
            }
            ...
            try {
                $R = \phpseclib3\Crypt\EC\Formats\Keys\PKCS1::extractPoint($R, $curve);
                $R = $this->curve->convertToInternal($R);
            } catch (\Exception $e) {
                return \false;
            }
            $S = \strrev($S);
            $S = new \phpseclib3\Math\BigInteger($S, 256);
        ...
        if (self::$engines['OpenSSL'] && \in_array($this->hash->getHash(), \openssl_get_md_methods())) {
            $sig = $format != 'ASN1' ? \phpseclib3\Crypt\EC\Formats\Signature\ASN1::save($r, $s) : $signature;
