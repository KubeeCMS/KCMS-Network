...
abstract class PSS extends \phpseclib3\Crypt\Common\Formats\Keys\PKCS8
{
    ...
    public static function load($key, $password = '')
    {
        ...
        if (isset($key[$type . 'KeyAlgorithm']['parameters'])) {
            $decoded = \phpseclib3\File\ASN1::decodeBER($key[$type . 'KeyAlgorithm']['parameters']);
            if ($decoded === \false) {
                throw new \UnexpectedValueException('Unable to decode parameters');
            }
            $params = \phpseclib3\File\ASN1::asn1map($decoded[0], \phpseclib3\File\ASN1\Maps\RSASSA_PSS_params::MAP);
        } else {
            $params = [];
        }
        if (isset($params['maskGenAlgorithm']['parameters'])) {
            $decoded = \phpseclib3\File\ASN1::decodeBER($params['maskGenAlgorithm']['parameters']);
            if ($decoded === \false) {
                throw new \UnexpectedValueException('Unable to decode parameters');
            }
            $params['maskGenAlgorithm']['parameters'] = \phpseclib3\File\ASN1::asn1map($decoded[0], \phpseclib3\File\ASN1\Maps\HashAlgorithm::MAP);
        } else {
    ...
    public static function savePSSParams(array $options)
    {
        ...
        */
        $params = ['trailerField' => new \phpseclib3\Math\BigInteger(1)];
        if (isset($options['hash'])) {
            $params['hashAlgorithm']['algorithm'] = 'id-' . $options['hash'];
        }
        if (isset($options['MGFHash'])) {
            $temp = ['algorithm' => 'id-' . $options['MGFHash']];
            $temp = \phpseclib3\File\ASN1::encodeDER($temp, \phpseclib3\File\ASN1\Maps\HashAlgorithm::MAP);
            $params['maskGenAlgorithm'] = ['algorithm' => 'id-mgf1', 'parameters' => new \phpseclib3\File\ASN1\Element($temp)];
        }
        if (isset($options['saltLength'])) {
            $params['saltLength'] = new \phpseclib3\Math\BigInteger($options['saltLength']);
        }
        return new \phpseclib3\File\ASN1\Element(\phpseclib3\File\ASN1::encodeDER($params, \phpseclib3\File\ASN1\Maps\RSASSA_PSS_params::MAP));
    }
}
