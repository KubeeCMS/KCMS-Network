...
abstract class MSBLOB
{
    ...
    public static function load($key, $password = '')
    {
        ...
        $key = \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::decode($key);
        if (!\is_string($key)) {
        ...
        \extract(\unpack('atype/aversion/vreserved/Valgo', \phpseclib3\Common\Functions\Strings::shift($key, 8)));
        /**
         * @var string $type
         * @var string $version
        ...
        \extract(\unpack('Vmagic/Vbitlen/a4pubexp', \phpseclib3\Common\Functions\Strings::shift($key, 12)));
        /**
         * @var integer $magic
         * @var integer $bitlen
         * @var string $pubexp
        ...
        $components[$components['isPublicKey'] ? 'publicExponent' : 'privateExponent'] = new \phpseclib3\Math\BigInteger(\strrev($pubexp), 256);
        // BYTE modulus[rsapubkey.bitlen/8]
        $components['modulus'] = new \phpseclib3\Math\BigInteger(\strrev(\phpseclib3\Common\Functions\Strings::shift($key, $bitlen / 8)), 256);
        if ($publickey) {
            return $components;
        }
        $components['isPublicKey'] = \false;
        // BYTE prime1[rsapubkey.bitlen/16]
        $components['primes'] = [1 => new \phpseclib3\Math\BigInteger(\strrev(\phpseclib3\Common\Functions\Strings::shift($key, $bitlen / 16)), 256)];
        // BYTE prime2[rsapubkey.bitlen/16]
        $components['primes'][] = new \phpseclib3\Math\BigInteger(\strrev(\phpseclib3\Common\Functions\Strings::shift($key, $bitlen / 16)), 256);
        // BYTE exponent1[rsapubkey.bitlen/16]
        $components['exponents'] = [1 => new \phpseclib3\Math\BigInteger(\strrev(\phpseclib3\Common\Functions\Strings::shift($key, $bitlen / 16)), 256)];
        // BYTE exponent2[rsapubkey.bitlen/16]
        $components['exponents'][] = new \phpseclib3\Math\BigInteger(\strrev(\phpseclib3\Common\Functions\Strings::shift($key, $bitlen / 16)), 256);
        // BYTE coefficient[rsapubkey.bitlen/16]
        $components['coefficients'] = [2 => new \phpseclib3\Math\BigInteger(\strrev(\phpseclib3\Common\Functions\Strings::shift($key, $bitlen / 16)), 256)];
        if (isset($components['privateExponent'])) {
            $components['publicExponent'] = $components['privateExponent'];
        }
        // BYTE privateExponent[rsapubkey.bitlen/8]
        $components['privateExponent'] = new \phpseclib3\Math\BigInteger(\strrev(\phpseclib3\Common\Functions\Strings::shift($key, $bitlen / 8)), 256);
        return $components;
    }
    /**
    ...
    public static function savePrivateKey(\phpseclib3\Math\BigInteger $n, \phpseclib3\Math\BigInteger $e, \phpseclib3\Math\BigInteger $d, array $primes, array $exponents, array $coefficients, $password = '')
    {
        ...
        if (!empty($password) && \is_string($password)) {
            throw new \phpseclib3\Exception\UnsupportedFormatException('MSBLOB private keys do not support encryption');
        }
        $n = \strrev($n->toBytes());
        ...
        return \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::encode($key);
    }
    /**
