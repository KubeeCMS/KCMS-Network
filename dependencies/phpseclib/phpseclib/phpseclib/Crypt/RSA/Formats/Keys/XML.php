...
abstract class XML
{
    ...
    public static function load($key, $password = '')
    {
        ...
        foreach ($keys as $key) {
            ...
            $value = new \phpseclib3\Math\BigInteger(\WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::decode($temp->item(0)->nodeValue), 256);
            switch ($key) {
                case 'modulus':
    ...
    public static function savePrivateKey(\phpseclib3\Math\BigInteger $n, \phpseclib3\Math\BigInteger $e, \phpseclib3\Math\BigInteger $d, array $primes, array $exponents, array $coefficients, $password = '')
    {
        ...
        if (!empty($password) && \is_string($password)) {
            throw new \phpseclib3\Exception\UnsupportedFormatException('XML private keys do not support encryption');
        }
        return "<RSAKeyPair>\r\n" . '  <Modulus>' . \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::encode($n->toBytes()) . "</Modulus>\r\n" . '  <Exponent>' . \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::encode($e->toBytes()) . "</Exponent>\r\n" . '  <P>' . \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::encode($primes[1]->toBytes()) . "</P>\r\n" . '  <Q>' . \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::encode($primes[2]->toBytes()) . "</Q>\r\n" . '  <DP>' . \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::encode($exponents[1]->toBytes()) . "</DP>\r\n" . '  <DQ>' . \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::encode($exponents[2]->toBytes()) . "</DQ>\r\n" . '  <InverseQ>' . \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::encode($coefficients[2]->toBytes()) . "</InverseQ>\r\n" . '  <D>' . \WP_Ultimo\Dependencies\ParagonIE\ConstantTime\Base64::encode($d->toBytes()) . "</D>\r\n" . '</RSAKeyPair>';
    }
    /**
