...
abstract class ASN1
{
    ...
    {
        if ($encoded instanceof \phpseclib3\File\ASN1\Element) {
    ...
    private static function decode_ber($encoded, $start = 0, $encoded_pos = 0)
    {
        ...
        switch ($tag) {
            ...
            case self::TYPE_ENUMERATED:
                $current['content'] = new \phpseclib3\Math\BigInteger(\substr($content, $content_pos), -256);
                break;
            case self::TYPE_REAL:
    ...
    public static function asn1map($decoded, $mapping, $special = [])
    {
        ...
        switch (\true) {
            case $mapping['type'] == self::TYPE_ANY:
                ...
                if (isset($decoded['constant']) || !\array_key_exists($intype, self::ANY_MAP) || \ord(self::$encoded[$decoded['start']]) & 0x20) {
                    return new \phpseclib3\File\ASN1\Element(\substr(self::$encoded, $decoded['start'], $decoded['length']));
                }
        ...
        switch ($decoded['type']) {
            case self::TYPE_SEQUENCE:
                ...
                foreach ($mapping['children'] as $key => $child) {
                    ...
                    if ($maymatch) {
                        ...
                        switch ($child['type']) {
                            case \phpseclib3\File\ASN1::TYPE_INTEGER:
                                $map[$key] = new \phpseclib3\Math\BigInteger($child['default']);
                                break;
                            //case self::TYPE_OBJECT_IDENTIFIER:
            ...
            case self::TYPE_ENUMERATED:
                ...
                if (isset($mapping['implicit'])) {
                    $temp = new \phpseclib3\Math\BigInteger($decoded['content'], -256);
                }
                if (isset($mapping['mapping'])) {
    ...
    {
        $length = \ord(\phpseclib3\Common\Functions\Strings::shift($string));
        if ($length & 0x80) {
            // definite length, long form
            $length &= 0x7f;
            $temp = \phpseclib3\Common\Functions\Strings::shift($string, $length);
    ...
    {
        if ($source instanceof \phpseclib3\File\ASN1\Element) {
        ...
        switch ($tag) {
            ...
            case self::TYPE_ENUMERATED:
                if (!isset($mapping['mapping'])) {
                    if (\is_numeric($source)) {
                        $source = new \phpseclib3\Math\BigInteger($source);
                    }
                    ...
                    $value = new \phpseclib3\Math\BigInteger($value);
            ...
            case self::TYPE_ANY:
                ...
                switch (\true) {
                    ...
                    case \is_int($source):
                    case $source instanceof \phpseclib3\Math\BigInteger:
    ...
    public static function decodeOID($content)
    {
        static $eighty;
        if (!$eighty) {
            $eighty = new \phpseclib3\Math\BigInteger(80);
        }
        $oid = array();
        $pos = 0;
        $len = \strlen($content);
        ...
        while ($pos < $len) {
            ...
            $n = $n->bitwise_or(new \phpseclib3\Math\BigInteger($temp & 0x7f));
            if (~$temp & 0x80) {
                $oid[] = $n;
                $n = new \phpseclib3\Math\BigInteger();
            }
    ...
    public static function encodeOID($source)
    {
        ...
        if (!$mask) {
            $mask = new \phpseclib3\Math\BigInteger(0x7f);
            $zero = new \phpseclib3\Math\BigInteger();
            $forty = new \phpseclib3\Math\BigInteger(40);
        }
        ...
        $first = new \phpseclib3\Math\BigInteger($part1);
        $first = $first->multiply($forty);
        $first = $first->add(new \phpseclib3\Math\BigInteger($part2));
        ...
        foreach ($parts as $part) {
            if (!$part) {
                ...
                $temp = '';
                $part = new \phpseclib3\Math\BigInteger($part);
