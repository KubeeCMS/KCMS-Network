...
final class Parser
{
    ...
    public function parseMessage($data)
    {
        ...
        $message = new \WP_Ultimo\Dependencies\React\Dns\Model\Message();
        $message->data = $data;
    ...
    private function parse($data, \WP_Ultimo\Dependencies\React\Dns\Model\Message $message)
    {
     ...
     * @return ?Query
     */
    private function parseQuestion(\WP_Ultimo\Dependencies\React\Dns\Model\Message $message)
    {
        ...
        return new \WP_Ultimo\Dependencies\React\Dns\Query\Query(\implode('.', $labels), $type, $class);
    }
    /**
     * @param Message $message
     * @return ?Record returns parsed Record on success or null if data is invalid/incomplete
     */
    private function parseRecord(\WP_Ultimo\Dependencies\React\Dns\Model\Message $message)
    {
        $consumed = $message->consumed;
        ...
        if (\WP_Ultimo\Dependencies\React\Dns\Model\Message::TYPE_A === $type) {
            if ($rdLength === 4) {
                $rdata = \inet_ntop(\substr($message->data, $consumed, $rdLength));
                $consumed += $rdLength;
            }
        } elseif (\WP_Ultimo\Dependencies\React\Dns\Model\Message::TYPE_AAAA === $type) {
            if ($rdLength === 16) {
                $rdata = \inet_ntop(\substr($message->data, $consumed, $rdLength));
                $consumed += $rdLength;
            }
        } elseif (\WP_Ultimo\Dependencies\React\Dns\Model\Message::TYPE_CNAME === $type || \WP_Ultimo\Dependencies\React\Dns\Model\Message::TYPE_PTR === $type || \WP_Ultimo\Dependencies\React\Dns\Model\Message::TYPE_NS === $type) {
            list($rdata, $consumed) = $this->readDomain($message->data, $consumed);
        } elseif (\WP_Ultimo\Dependencies\React\Dns\Model\Message::TYPE_TXT === $type) {
            $rdata = array();
        ...
        } elseif (\WP_Ultimo\Dependencies\React\Dns\Model\Message::TYPE_MX === $type) {
            if ($rdLength > 2) {
        ...
        } elseif (\WP_Ultimo\Dependencies\React\Dns\Model\Message::TYPE_SRV === $type) {
            if ($rdLength > 6) {
        ...
        } elseif (\WP_Ultimo\Dependencies\React\Dns\Model\Message::TYPE_SSHFP === $type) {
            if ($rdLength > 2) {
        ...
        } elseif (\WP_Ultimo\Dependencies\React\Dns\Model\Message::TYPE_SOA === $type) {
        ...
        } elseif (\WP_Ultimo\Dependencies\React\Dns\Model\Message::TYPE_OPT === $type) {
            $rdata = array();
            while (isset($message->data[$consumed + 4 - 1])) {
                ...
                if ($code === \WP_Ultimo\Dependencies\React\Dns\Model\Message::OPT_TCP_KEEPALIVE && $value === '') {
                    $value = null;
                } elseif ($code === \WP_Ultimo\Dependencies\React\Dns\Model\Message::OPT_TCP_KEEPALIVE && $length === 2) {
                    list($value) = \array_values(\unpack('n', $value));
                    $value = \round($value * 0.1, 1);
                } elseif ($code === \WP_Ultimo\Dependencies\React\Dns\Model\Message::OPT_TCP_KEEPALIVE) {
                    break;
                }
                $rdata[$code] = $value;
                $consumed += 4 + $length;
            }
        } elseif (\WP_Ultimo\Dependencies\React\Dns\Model\Message::TYPE_CAA === $type) {
            if ($rdLength > 3) {
        ...
        $message->consumed = $consumed;
        return new \WP_Ultimo\Dependencies\React\Dns\Model\Record($name, $type, $class, $ttl, $rdata);
    }
