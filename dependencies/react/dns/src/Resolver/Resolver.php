...
use WP_Ultimo\Dependencies\React\Dns\RecordNotFoundException;
/**
 * @see ResolverInterface for the base interface
 */
final class Resolver implements \WP_Ultimo\Dependencies\React\Dns\Resolver\ResolverInterface
{
    private $executor;
    public function __construct(\WP_Ultimo\Dependencies\React\Dns\Query\ExecutorInterface $executor)
    {
        $this->executor = $executor;
    }
    public function resolve($domain)
    {
        return $this->resolveAll($domain, \WP_Ultimo\Dependencies\React\Dns\Model\Message::TYPE_A)->then(function (array $ips) {
            return $ips[\array_rand($ips)];
        });
    }
    public function resolveAll($domain, $type)
    {
        $query = new \WP_Ultimo\Dependencies\React\Dns\Query\Query($domain, $type, \WP_Ultimo\Dependencies\React\Dns\Model\Message::CLASS_IN);
        $that = $this;
        return $this->executor->query($query)->then(function (\WP_Ultimo\Dependencies\React\Dns\Model\Message $response) use($query, $that) {
     ...
     * @internal
     */
    public function extractValues(\WP_Ultimo\Dependencies\React\Dns\Query\Query $query, \WP_Ultimo\Dependencies\React\Dns\Model\Message $response)
    {
        // reject if response code indicates this is an error response message
        $code = $response->rcode;
        if ($code !== \WP_Ultimo\Dependencies\React\Dns\Model\Message::RCODE_OK) {
            switch ($code) {
                case \WP_Ultimo\Dependencies\React\Dns\Model\Message::RCODE_FORMAT_ERROR:
                    $message = 'Format Error';
                    break;
                case \WP_Ultimo\Dependencies\React\Dns\Model\Message::RCODE_SERVER_FAILURE:
                    $message = 'Server Failure';
                    break;
                case \WP_Ultimo\Dependencies\React\Dns\Model\Message::RCODE_NAME_ERROR:
                    $message = 'Non-Existent Domain / NXDOMAIN';
                    break;
                case \WP_Ultimo\Dependencies\React\Dns\Model\Message::RCODE_NOT_IMPLEMENTED:
                    $message = 'Not Implemented';
                    break;
                case \WP_Ultimo\Dependencies\React\Dns\Model\Message::RCODE_REFUSED:
                    $message = 'Refused';
                    break;
                default:
                    $message = 'Unknown error response code ' . $code;
            }
            throw new \WP_Ultimo\Dependencies\React\Dns\RecordNotFoundException('DNS query for ' . $query->name . ' returned an error response (' . $message . ')', $code);
        }
        ...
        if (0 === \count($addresses)) {
            throw new \WP_Ultimo\Dependencies\React\Dns\RecordNotFoundException('DNS query for ' . $query->name . ' did not return a valid answer (NOERROR / NODATA)');
        }
    ...
    private function valuesByNameAndType(array $answers, $name, $type)
    {
        ...
        $cnameRecords = $this->filterByType($named, \WP_Ultimo\Dependencies\React\Dns\Model\Message::TYPE_CNAME);
        if ($cnameRecords) {
