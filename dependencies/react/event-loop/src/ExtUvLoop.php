...
final class ExtUvLoop implements \React\EventLoop\LoopInterface
{
    ...
    public function addSignal($signal, $listener)
    {
        ...
        if (!isset($this->signalEvents[$signal])) {
            ...
            $this->signalEvents[$signal] = \WP_Ultimo\Dependencies\uv_signal_init($this->uv);
            \WP_Ultimo\Dependencies\uv_signal_start($this->signalEvents[$signal], function () use($signals, $signal) {
                $signals->call($signal);
            }, $signal);
        }
    }
    ...
    {
        if (!isset($this->streamEvents[(int) $stream])) {
            $this->streamEvents[(int) $stream] = \WP_Ultimo\Dependencies\uv_poll_init_socket($this->uv, $stream);
        }
