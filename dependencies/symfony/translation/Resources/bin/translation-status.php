...
foreach ($config['original_files'] as $originalFilePath) {
    $translationFilePaths = \WP_Ultimo\Dependencies\findTranslationFiles($originalFilePath, $config['locale_to_analyze']);
    $translationStatus = \WP_Ultimo\Dependencies\calculateTranslationStatus($originalFilePath, $translationFilePaths);
    $totalMissingTranslations += \array_sum(\array_map(function ($translation) {
        return \count($translation['missingKeys']);
    }, \array_values($translationStatus)));
    \WP_Ultimo\Dependencies\printTranslationStatus($originalFilePath, $translationStatus, $config['verbose_output']);
}
...
function findTranslationFiles($originalFilePath, $localeToAnalyze)
{
    ...
    foreach ($translationFiles as $filePath) {
        $locale = \WP_Ultimo\Dependencies\extractLocaleFromFilePath($filePath);
...
{
    $translationStatus = [];
    $allTranslationKeys = \WP_Ultimo\Dependencies\extractTranslationKeys($originalFilePath);
    foreach ($translationFilePaths as $locale => $translationPath) {
        $translatedKeys = \WP_Ultimo\Dependencies\extractTranslationKeys($translationPath);
...
{
    \WP_Ultimo\Dependencies\printTitle($originalFilePath);
    \WP_Ultimo\Dependencies\printTable($translationStatus, $verboseOutput);
