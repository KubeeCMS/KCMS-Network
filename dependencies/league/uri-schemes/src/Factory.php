...
class Factory
{
     ...
     * @var string[]
     */
    protected $map = ['http' => Http::class, 'https' => Http::class, 'ftp' => Ftp::class, 'ws' => Ws::class, 'wss' => Ws::class, 'data' => Data::class, 'file' => File::class];
    /**
     * Dot segments.
     *
     * @var array
     ...
     * @var array
     */
    protected static $uri_interfaces = [DeprecatedLeagueUriInterface::class, UriInterface::class, Psr7UriInterface::class];
    /**
     * new instance.
     *
    ...
    protected function getClassName($scheme, $base_uri = null) : string
    {
        ...
        if (isset($base_uri) && in_array($scheme, [$base_uri->getScheme(), ''], \true)) {
    ...
    {
        $uri = (new ReflectionClass($className))->newInstanceWithoutConstructor()->withHost($components['host'] ?? '')->withPort($components['port'] ?? null)->withUserInfo($components['user'] ?? '', $components['pass'] ?? null)->withScheme($components['scheme'] ?? '');
        $path = $components['path'] ?? '';
        if ('' !== $uri->getAuthority() && '' !== $path && '/' !== $path[0]) {
            $path = '/' . $path;
        }
        return $uri->withPath($path)->withQuery($components['query'] ?? '')->withFragment($components['fragment'] ?? '');
    }
    /**
    ...
    {
        if ('' !== $uri->getScheme()) {
            return $uri->withPath($this->removeDotSegments($uri->getPath()));
        }
        if ('' !== $uri->getAuthority()) {
            return $uri->withScheme($base_uri->getScheme())->withPath($this->removeDotSegments($uri->getPath()));
        }
        ...
        return $uri->withPath($this->removeDotSegments($uri_path))->withQuery($uri_query)->withHost($base_uri->getHost())->withPort($base_uri->getPort())->withUserInfo($base_uri_user, $base_uri_pass)->withScheme($base_uri->getScheme());
    }
    /**
