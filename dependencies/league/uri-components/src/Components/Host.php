...
namespace WP_Ultimo\Dependencies\League\Uri\Components;

use WP_Ultimo\Dependencies\League\Uri\PublicSuffix\Cache;
use WP_Ultimo\Dependencies\League\Uri\PublicSuffix\CurlHttpClient;
use WP_Ultimo\Dependencies\League\Uri\PublicSuffix\ICANNSectionManager;
use WP_Ultimo\Dependencies\League\Uri\PublicSuffix\Rules;
use Traversable;
/**
...
class Host extends AbstractHierarchicalComponent implements ComponentInterface
{
    ...
    {
        $host = static::createFromLabels($properties['data'], $properties['is_absolute'], $properties['resolver'] ?? null);
    ...
    {
        if (null === $host) {
            return ['data' => [], 'ip_version' => null, 'has_zone_identifier' => \false, 'host_as_domain_name' => \false, 'is_absolute' => self::IS_RELATIVE];
        }
        if ('' === $host) {
            return ['data' => [''], 'ip_version' => null, 'has_zone_identifier' => \false, 'host_as_domain_name' => \false, 'is_absolute' => self::IS_RELATIVE];
        }
        ...
        if (\filter_var($host, \FILTER_VALIDATE_IP, \FILTER_FLAG_IPV4)) {
            return ['data' => [$host], 'ip_version' => '4', 'has_zone_identifier' => \false, 'host_as_domain_name' => \false, 'is_absolute' => self::IS_RELATIVE];
        }
        ...
        if ($this->isValidDomain($reg_name)) {
            ...
            return ['data' => \array_reverse(\explode('.', $reg_name)), 'ip_version' => null, 'has_zone_identifier' => \false, 'host_as_domain_name' => \true, 'is_absolute' => $is_absolute];
        }
        if ($this->isValidRegisteredName($reg_name)) {
            return ['data' => [$reg_name], 'ip_version' => null, 'has_zone_identifier' => \false, 'host_as_domain_name' => \false, 'is_absolute' => self::IS_RELATIVE];
        }
        if ($this->isValidIpv6Hostname($host)) {
            return ['data' => [$host], 'ip_version' => '6', 'has_zone_identifier' => \false !== \strpos($host, '%'), 'host_as_domain_name' => \false, 'is_absolute' => self::IS_RELATIVE];
        }
        if ($this->isValidIpFuture($host)) {
            \preg_match('/^v(?<version>[A-F0-9]+)\\./', \substr($host, 1, -1), $matches);
            return ['data' => [$host], 'ip_version' => $matches['version'], 'is_absolute' => self::IS_RELATIVE, 'has_zone_identifier' => \false, 'host_as_domain_name' => \false];
        }
    ...
    private function isValidIpFuture(string $ipfuture) : bool
    {
        ...
        $/ix';
        return \preg_match($pattern, \substr($ipfuture, 1, -1), $matches) && !\in_array($matches['version'], ['4', '6'], \true);
    }
    /**
    ...
    {
        $this->lazyloadInfo();
        return \array_merge(['component' => $this->getContent(), 'labels' => $this->data, 'is_absolute' => (bool) $this->is_absolute], $this->hostname);
    }
    /**
    ...
    protected function lazyloadInfo()
    {
        ...
        if (!$this->host_as_domain_name) {
            $this->hostname = $this->hostname = ['isPublicSuffixValid' => \false, 'publicSuffix' => '', 'registrableDomain' => '', 'subDomain' => ''];
            return;
        }
        ...
        $this->hostname = ['isPublicSuffixValid' => $domain->isValid(), 'publicSuffix' => (string) $domain->getPublicSuffix(), 'registrableDomain' => (string) $domain->getRegistrableDomain(), 'subDomain' => (string) $domain->getSubDomain()];
    }
    /**
    ...
    public function keys(...$args) : array
    {
        ...
        return \array_keys($this->data, $this->toIdn($this->validateString($args[0])), \true);
    }
    /**
    ...
    public function withPublicSuffix(string $host) : self
    {
        ...
        $new = self::createFromLabels(\array_merge($new, \array_slice($this->data, $offset)), $this->is_absolute, $this->resolver);
        $new->lazyloadInfo();
        return $new;
    }
    /**
    ...
    protected function validate(string $host = null) : array
    {
        ...
        if (\filter_var($host, \FILTER_VALIDATE_IP, \FILTER_FLAG_IPV4)) {
            $this->host_as_ipv4 = \true;
            return [$host];
        }
        if ($this->isValidIpv6Hostname($host)) {
            $this->host_as_ipv6 = \true;
    ...
    public function withSubDomain(string $host) : self
    {
        ...
        $new = self::createFromLabels(\array_merge(\array_slice($this->data, 0, $offset), $new), $this->is_absolute, $this->resolver);
        $new->lazyloadInfo();
        return $new;
    }
    /**
    ...
    protected function isValidHostname(string $host) : bool
    {
        ...
        return 127 > \count($labels) && 253 > \strlen(\implode('.', $labels)) && $labels === \array_filter($labels, [$this, 'isValidLabel']);
    }
    /**
    ...
    {
        return \is_string($label) && '' != $label && 63 >= \strlen($label) && \strlen($label) == \strspn($label, self::STARTING_LABEL_CHARS . '-_~' . self::SUB_DELIMITERS);
    }
    /**
