...
class DataPath extends AbstractComponent
{
    ...
    public static function createFromPath(string $path) : self
    {
        ...
        return new static(static::format(\str_replace(' ', '', (new \finfo(\FILEINFO_MIME))->file($path)), '', \true, \base64_encode(\file_get_contents($path))));
    }
    /**
     * new instance.
     *
     ...
     *
     */
    protected static function format(string $mimetype, string $parameters, bool $is_binary_data, string $data) : string
    {
        if ('' != $parameters) {
    ...
    {
        return ['component' => $this->getContent(), 'mimetype' => $this->mimetype, 'parameters' => $this->parameters, 'is_binary' => $this->is_binary_data, 'data' => $this->document];
    }
    /**
    ...
    public function toBinary() : self
    {
        ...
        return new static($this->format($this->mimetype, $this->getParameters(), !$this->is_binary_data, \base64_encode(\rawurldecode($this->document))));
    }
    /**
    ...
    public function toAscii() : self
    {
        ...
        return new static($this->format($this->mimetype, $this->getParameters(), !$this->is_binary_data, \rawurlencode(\base64_decode($this->document))));
    }
    /**
