...
{
    const ENCODING_LIST = [self::RFC1738_ENCODING => 1, self::RFC3986_ENCODING => 1, self::RFC3987_ENCODING => 1, self::NO_ENCODING => 1];
    /**
     * @var callable
     */
    private $decoder;
     ...
     *
     */
    public function parse(string $str, string $separator = '&', int $enc_type = self::RFC3986_ENCODING) : array
    {
    ...
    public function convert($pairs) : array
    {
        ...
        foreach ($pairs as $name => $value) {
            ...
            foreach ($value as $val) {
                $this->extractPhpVariable(\trim((string) $name), $this->normalize($val), $data);
            }
        }
        return $data;
    }
