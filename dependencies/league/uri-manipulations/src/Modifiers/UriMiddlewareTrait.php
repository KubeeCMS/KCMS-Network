...
use WP_Ultimo\Dependencies\League\Uri\Components\DataPath;
use WP_Ultimo\Dependencies\League\Uri\Components\HierarchicalPath;
use WP_Ultimo\Dependencies\League\Uri\Components\Host;
use WP_Ultimo\Dependencies\League\Uri\Components\Path;
use WP_Ultimo\Dependencies\League\Uri\Components\Query;
use WP_Ultimo\Dependencies\League\Uri\Interfaces\Uri;
...
trait UriMiddlewareTrait
{
     ...
     * @return Uri|UriInterface
     */
    protected abstract function execute($uri);
    /**
     * validate a string
     *
    ...
    {
        return $this->filterDataPath('text/plain;charset=us-ascii,')->withParameters($parameters)->getParameters();
    }
    /**
    ...
    protected function filterExtension(string $extension) : string
    {
        ...
        if (0 === \strpos($extension, '.') || \false !== \strpos($extension, '/')) {
            throw new Exception('extension must be string sequence without a leading `.` and the path separator `/` characters');
        }
