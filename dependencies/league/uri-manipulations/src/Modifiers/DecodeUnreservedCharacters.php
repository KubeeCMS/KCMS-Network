...
class DecodeUnreservedCharacters implements UriMiddlewareInterface
{
    ...
    {
        $decoded = \preg_replace_callback(self::UNRESERVED_CHARS_ENCODED, function (array $matches) {
            return \rawurldecode($matches[0]);
        }, [$uri->getPath(), $uri->getQuery(), $uri->getFragment()]);
        return $uri->withPath($decoded[0])->withQuery($decoded[1])->withFragment($decoded[2]);
    }
}
