...
class Resolve implements UriMiddlewareInterface
{
    ...
    {
        if (!$base_uri instanceof UriInterface && !$base_uri instanceof Psr7UriInterface && !$base_uri instanceof DeprecatedLeagueUriInterface) {
    ...
    protected function execute($uri)
    {
        ...
        if ('' !== $uri->getScheme()) {
            return $uri->withPath((string) $this->filterPath($target_path)->withoutDotSegments());
        }
        if ('' !== $uri->getAuthority()) {
            return $uri->withScheme($this->base_uri->getScheme())->withPath((string) $this->filterPath($target_path)->withoutDotSegments());
        }
        list($user, $pass) = \explode(':', $this->base_uri->getUserInfo(), 2) + ['', null];
        $components = $this->resolvePathAndQuery($target_path, $uri->getQuery());
        return $uri->withPath($this->formatPath($components['path']))->withQuery($components['query'])->withHost($this->base_uri->getHost())->withPort($this->base_uri->getPort())->withUserInfo($user, $pass)->withScheme($this->base_uri->getScheme());
    }
    /**
