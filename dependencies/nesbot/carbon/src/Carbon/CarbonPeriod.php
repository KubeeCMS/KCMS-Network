...
use WP_Ultimo\Dependencies\Carbon\Exceptions\InvalidPeriodParameterException;
use WP_Ultimo\Dependencies\Carbon\Exceptions\NotACarbonClassException;
use WP_Ultimo\Dependencies\Carbon\Exceptions\NotAPeriodException;
use WP_Ultimo\Dependencies\Carbon\Exceptions\UnknownGetterException;
use WP_Ultimo\Dependencies\Carbon\Exceptions\UnknownMethodException;
use WP_Ultimo\Dependencies\Carbon\Exceptions\UnreachableException;
...
class CarbonPeriod implements \Iterator, \Countable, \JsonSerializable
{
     ...
     * @var string
     */
    protected $dateClass = \WP_Ultimo\Dependencies\Carbon\Carbon::class;
    /**
    ...
    public static function make($var)
    {
        ...
        } catch (\WP_Ultimo\Dependencies\Carbon\Exceptions\NotAPeriodException $e) {
    ...
    public static function instance($period)
    {
        ...
        $type = \gettype($period);
        throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\NotAPeriodException('Argument 1 passed to ' . $class . '::' . __METHOD__ . '() ' . 'must be an instance of DatePeriod or ' . $class . ', ' . ($type === 'object' ? 'instance of ' . \get_class($period) : $type) . ' given.');
    }
    /**
    ...
    protected static function parseIso8601($iso)
    {
        ...
        foreach (\explode('/', $iso) as $key => $part) {
            ...
            } elseif ($interval === null && ($parsed = \WP_Ultimo\Dependencies\Carbon\CarbonInterval::make($part))) {
                $interval = $part;
            } elseif ($start === null && ($parsed = \WP_Ultimo\Dependencies\Carbon\Carbon::make($part))) {
                $start = $part;
            } elseif ($end === null && ($parsed = \WP_Ultimo\Dependencies\Carbon\Carbon::make(static::addMissingParts($start, $part)))) {
                $end = $part;
            } else {
                throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\InvalidPeriodParameterException("Invalid ISO 8601 specification: {$iso}.");
            }
            $result[] = $parsed;
        }
        return $result;
    ...
    public function __construct(...$arguments)
    {
        ...
        foreach ($arguments as $argument) {
            if ($this->dateInterval === null && (\is_string($argument) && \preg_match('/^(\\d(\\d(?![\\/-])|[^\\d\\/-]([\\/-])?)*|P[T0-9].*|(?:\\h*\\d+(?:\\.\\d+)?\\h*[a-z]+)+)$/i', $argument) || $argument instanceof \DateInterval || $argument instanceof \Closure) && ($parsed = @\WP_Ultimo\Dependencies\Carbon\CarbonInterval::make($argument))) {
                $this->setDateInterval($parsed);
            } elseif ($this->startDate === null && ($parsed = \WP_Ultimo\Dependencies\Carbon\Carbon::make($argument))) {
                $this->setStartDate($parsed);
            } elseif ($this->endDate === null && ($parsed = \WP_Ultimo\Dependencies\Carbon\Carbon::make($argument))) {
                ...
                throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\InvalidPeriodParameterException('Invalid constructor parameters.');
            }
        }
        if ($this->startDate === null) {
            $this->setStartDate(\WP_Ultimo\Dependencies\Carbon\Carbon::now());
        }
        if ($this->dateInterval === null) {
            $this->setDateInterval(\WP_Ultimo\Dependencies\Carbon\CarbonInterval::day());
    ...
    public function __isset(string $name) : bool
    {
        try {
            $this->__get($name);
        } catch (\WP_Ultimo\Dependencies\Carbon\Exceptions\UnknownGetterException|\ReflectionException $e) {
            return \false;
        }
        return \true;
    }
    ...
    {
        if (!\is_a($dateClass, \WP_Ultimo\Dependencies\Carbon\CarbonInterface::class, \true)) {
            throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\NotACarbonClassException($dateClass);
        }
        $this->dateClass = $dateClass;
        if (\is_a($dateClass, \WP_Ultimo\Dependencies\Carbon\Carbon::class, \true)) {
            $this->toggleOptions(static::IMMUTABLE, \false);
        } elseif (\is_a($dateClass, \WP_Ultimo\Dependencies\Carbon\CarbonImmutable::class, \true)) {
    ...
    {
        if (!($interval = \WP_Ultimo\Dependencies\Carbon\CarbonInterval::make($interval))) {
            throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\InvalidIntervalException('Invalid interval.');
        }
        if ($interval->spec() === 'PT0S' && !$interval->f && !$interval->getStep()) {
            throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\InvalidIntervalException('Empty interval is not accepted.');
        }
    ...
    public function setOptions($options)
    {
        if (!\is_int($options) && !\is_null($options)) {
            throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\InvalidPeriodParameterException('Invalid options.');
        }
    ...
    public function setRecurrences($recurrences)
    {
        if (!\is_numeric($recurrences) && !\is_null($recurrences) || $recurrences < 0) {
            throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\InvalidPeriodParameterException('Invalid number of recurrences.');
        }
        if ($recurrences === null) {
    ...
    public function setStartDate($date, $inclusive = null)
    {
        if (!($date = \call_user_func([$this->dateClass, 'make'], $date))) {
            throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\InvalidPeriodDateException('Invalid start date.');
        }
        $this->startDate = $date;
        if ($inclusive !== null) {
    ...
    public function setEndDate($date, $inclusive = null)
    {
        if (!\is_null($date) && !($date = \call_user_func([$this->dateClass, 'make'], $date))) {
            throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\InvalidPeriodDateException('Invalid end date.');
        }
        if (!$date) {
    ...
    public function __call($method, $parameters)
    {
        ...
        switch ($method) {
            ...
            case 'sinceNow':
                return $this->setStartDate(new \WP_Ultimo\Dependencies\Carbon\Carbon(), $first);
            case 'end':
            case 'until':
                return $this->setEndDate($first, $second);
            case 'untilNow':
                return $this->setEndDate(new \WP_Ultimo\Dependencies\Carbon\Carbon(), $first);
            case 'dates':
            case 'between':
            ...
            case 'second':
                return $this->setDateInterval(\call_user_func(
                    ...
                    [$this->isDefaultInterval ? new \WP_Ultimo\Dependencies\Carbon\CarbonInterval('PT0S') : $this->dateInterval, $method],
                    \count($parameters) === 0 ? 1 : $first
                ));
        }
        if ($this->localStrictModeEnabled ?? \WP_Ultimo\Dependencies\Carbon\Carbon::isStrictModeEnabled()) {
            throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\UnknownMethodException($method);
        }
        return $this;
    }
    /**
    ...
    {
        if ($this->getOptions() & static::IMMUTABLE && $this->dateClass === \WP_Ultimo\Dependencies\Carbon\Carbon::class) {
            $this->setDateClass(\WP_Ultimo\Dependencies\Carbon\CarbonImmutable::class);
        } elseif (!($this->getOptions() & static::IMMUTABLE) && $this->dateClass === \WP_Ultimo\Dependencies\Carbon\CarbonImmutable::class) {
            $this->setDateClass(\WP_Ultimo\Dependencies\Carbon\Carbon::class);
        }
        $this->validationResult = null;
    }
    /**
     ...
     * @return CarbonInterface
     */
    protected function prepareForReturn(\WP_Ultimo\Dependencies\Carbon\CarbonInterface $date)
    {
    ...
    protected function incrementCurrentDateUntilValid()
    {
        ...
        do {
            ...
            if (++$attempts > static::NEXT_MAX_ATTEMPTS) {
                throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\UnreachableException('Could not find next valid date.');
            }
