...
trait Date
{
    ...
    protected static $days = [
        // @call isDayOfWeek
        \WP_Ultimo\Dependencies\Carbon\CarbonInterface::SUNDAY => 'Sunday',
        // @call isDayOfWeek
        \WP_Ultimo\Dependencies\Carbon\CarbonInterface::MONDAY => 'Monday',
        // @call isDayOfWeek
        \WP_Ultimo\Dependencies\Carbon\CarbonInterface::TUESDAY => 'Tuesday',
        // @call isDayOfWeek
        \WP_Ultimo\Dependencies\Carbon\CarbonInterface::WEDNESDAY => 'Wednesday',
        // @call isDayOfWeek
        \WP_Ultimo\Dependencies\Carbon\CarbonInterface::THURSDAY => 'Thursday',
        // @call isDayOfWeek
        \WP_Ultimo\Dependencies\Carbon\CarbonInterface::FRIDAY => 'Friday',
        // @call isDayOfWeek
        \WP_Ultimo\Dependencies\Carbon\CarbonInterface::SATURDAY => 'Saturday',
    ];
    /**
    ...
    {
        return \WP_Ultimo\Dependencies\Carbon\CarbonTimeZone::instance($object, $objectDump);
    }
    /**
    ...
    {
        return \WP_Ultimo\Dependencies\Carbon\CarbonTimeZone::instance(parent::getTimezone());
    }
    /**
    ...
    protected static function expectDateTime($date, $other = [])
    {
        ...
        if (!$date instanceof \DateTime && !$date instanceof \DateTimeInterface) {
            throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\InvalidTypeException($message . 'DateTime or DateTimeInterface, ' . (\is_object($date) ? \get_class($date) : \gettype($date)) . ' given');
        }
    }
    /**
    ...
    public function carbonize($date = null)
    {
        ...
        if ($date instanceof \DatePeriod || $date instanceof \WP_Ultimo\Dependencies\Carbon\CarbonPeriod) {
    ...
    public function get($name)
    {
        ...
        switch (\true) {
            ...
            case $name === 'timezone' || $name === 'tz':
                return \WP_Ultimo\Dependencies\Carbon\CarbonTimeZone::instance($this->getTimezone());
            // @property-read string $timezoneName the current timezone name
            // @property-read string $tzName alias of $timezoneName
    ...
    {
        if ($this->isImmutable()) {
            throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\ImmutableException(\sprintf('%s class', static::class));
        }
        if (\is_array($name)) {
        ...
        switch ($name) {
            ...
            default:
                ...
                if ($this->localStrictModeEnabled ?? static::isStrictModeEnabled()) {
                    throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\UnknownSetterException($name);
                }
                $this->{$name} = $value;
        }
        return $this;
    ...
    public function setUnitNoOverflow($valueUnit, $value, $overflowUnit)
    {
        try {
            ...
            throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\UnknownUnitException($valueUnit, 0, $exception);
        }
    }
    /**
    ...
    public static function getIsoUnits()
    {
        static $units = null;
        if ($units === null) {
            $units = ['OD' => ['getAltNumber', ['day']], 'OM' => ['getAltNumber', ['month']], 'OY' => ['getAltNumber', ['year']], 'OH' => ['getAltNumber', ['hour']], 'Oh' => ['getAltNumber', ['h']], 'Om' => ['getAltNumber', ['minute']], 'Os' => ['getAltNumber', ['second']], 'D' => 'day', 'DD' => ['rawFormat', ['d']], 'Do' => ['ordinal', ['day', 'D']], 'd' => 'dayOfWeek', 'dd' => function (\WP_Ultimo\Dependencies\Carbon\CarbonInterface $date, $originalFormat = null) {
                return $date->getTranslatedMinDayName($originalFormat);
            }, 'ddd' => function (\WP_Ultimo\Dependencies\Carbon\CarbonInterface $date, $originalFormat = null) {
                return $date->getTranslatedShortDayName($originalFormat);
            }, 'dddd' => function (\WP_Ultimo\Dependencies\Carbon\CarbonInterface $date, $originalFormat = null) {
                return $date->getTranslatedDayName($originalFormat);
            }, 'DDD' => 'dayOfYear', 'DDDD' => ['getPaddedUnit', ['dayOfYear', 3]], 'DDDo' => ['ordinal', ['dayOfYear', 'DDD']], 'e' => ['weekday', []], 'E' => 'dayOfWeekIso', 'H' => ['rawFormat', ['G']], 'HH' => ['rawFormat', ['H']], 'h' => ['rawFormat', ['g']], 'hh' => ['rawFormat', ['h']], 'k' => 'noZeroHour', 'kk' => ['getPaddedUnit', ['noZeroHour']], 'hmm' => ['rawFormat', ['gi']], 'hmmss' => ['rawFormat', ['gis']], 'Hmm' => ['rawFormat', ['Gi']], 'Hmmss' => ['rawFormat', ['Gis']], 'm' => 'minute', 'mm' => ['rawFormat', ['i']], 'a' => 'meridiem', 'A' => 'upperMeridiem', 's' => 'second', 'ss' => ['getPaddedUnit', ['second']], 'S' => function (\WP_Ultimo\Dependencies\Carbon\CarbonInterface $date) {
                return \strval((string) \floor($date->micro / 100000));
            }, 'SS' => function (\WP_Ultimo\Dependencies\Carbon\CarbonInterface $date) {
                return \str_pad((string) \floor($date->micro / 10000), 2, '0', \STR_PAD_LEFT);
            }, 'SSS' => function (\WP_Ultimo\Dependencies\Carbon\CarbonInterface $date) {
                return \str_pad((string) \floor($date->micro / 1000), 3, '0', \STR_PAD_LEFT);
            }, 'SSSS' => function (\WP_Ultimo\Dependencies\Carbon\CarbonInterface $date) {
                return \str_pad((string) \floor($date->micro / 100), 4, '0', \STR_PAD_LEFT);
            }, 'SSSSS' => function (\WP_Ultimo\Dependencies\Carbon\CarbonInterface $date) {
                return \str_pad((string) \floor($date->micro / 10), 5, '0', \STR_PAD_LEFT);
            }, 'SSSSSS' => ['getPaddedUnit', ['micro', 6]], 'SSSSSSS' => function (\WP_Ultimo\Dependencies\Carbon\CarbonInterface $date) {
                return \str_pad((string) \floor($date->micro * 10), 7, '0', \STR_PAD_LEFT);
            }, 'SSSSSSSS' => function (\WP_Ultimo\Dependencies\Carbon\CarbonInterface $date) {
                return \str_pad((string) \floor($date->micro * 100), 8, '0', \STR_PAD_LEFT);
            }, 'SSSSSSSSS' => function (\WP_Ultimo\Dependencies\Carbon\CarbonInterface $date) {
                return \str_pad((string) \floor($date->micro * 1000), 9, '0', \STR_PAD_LEFT);
            }, 'M' => 'month', 'MM' => ['rawFormat', ['m']], 'MMM' => function (\WP_Ultimo\Dependencies\Carbon\CarbonInterface $date, $originalFormat = null) {
    ...
    public function isoFormat(string $format, string $originalFormat = null) : string
    {
        ...
        for ($i = 0; $i < $length; $i++) {
            ...
            if (\preg_match('/^' . \WP_Ultimo\Dependencies\Carbon\CarbonInterface::ISO_FORMAT_REGEXP . '/', $input, $match)) {
                $code = $match[0];
                if ($units === null) {
    ...
    public static function __callStatic($method, $parameters)
    {
        if (!static::hasMacro($method)) {
            ...
            if (static::isStrictModeEnabled()) {
                throw new \WP_Ultimo\Dependencies\Carbon\Exceptions\UnknownMethodException(\sprintf('%s::%s', static::class, $method));
            }
            return null;
        }
    ...
    public function __call($method, $parameters)
    {
        ...
        $diffSyntaxModes = [
            // @call diffForHumans
            'Absolute' => \WP_Ultimo\Dependencies\Carbon\CarbonInterface::DIFF_ABSOLUTE,
            // @call diffForHumans
            'Relative' => \WP_Ultimo\Dependencies\Carbon\CarbonInterface::DIFF_RELATIVE_AUTO,
            // @call diffForHumans
            'RelativeToNow' => \WP_Ultimo\Dependencies\Carbon\CarbonInterface::DIFF_RELATIVE_TO_NOW,
            // @call diffForHumans
            'RelativeToOther' => \WP_Ultimo\Dependencies\Carbon\CarbonInterface::DIFF_RELATIVE_TO_OTHER,
        ];
        ...
        if ($sixFirstLetters === 'isSame') {
            ...
            } catch (\WP_Ultimo\Dependencies\Carbon\Exceptions\BadComparisonUnitException $exception) {
                // Try next
            }
        }
        if (\substr($unit, 0, 9) === 'isCurrent') {
            ...
            } catch (\WP_Ultimo\Dependencies\Carbon\Exceptions\BadComparisonUnitException|\BadMethodCallException $exception) {
                // Try next
            }
        }
