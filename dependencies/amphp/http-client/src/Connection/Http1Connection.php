...
final class Http1Connection implements Connection
{
    ...
    public function __construct(Socket $socket, int $timeoutGracePeriod)
    {
        ...
        $this->estimatedClose = (int) ((\microtime(\true) + self::MAX_KEEP_ALIVE_TIMEOUT) * 1000);
    }
    ...
    public function getStream(Request $request) : Promise
    {
        ...
        $this->busy = \true;
        return new Success(new HttpStream($this, \Closure::fromCallable([$this, 'request']), \Closure::fromCallable([$this, 'release'])));
    }
    private function hasStreamFor(Request $request) : bool
    {
        return !$this->busy && !$this->socket->isClosed() && ($this->getRemainingTime() > $this->timeoutGracePeriod || $request->isIdempotent());
    }
    /** @inheritdoc */
     ...
     */
    private function readResponse(Request $request, CancellationToken $originalCancellation, CancellationToken $readingCancellation) : \Generator
    {
        ...
        try {
            while (null !== ($chunk = (yield $this->socket->read()))) {
                ...
                // Read body async
                asyncCall(function () use($parser, $request, $response, $bodyEmitter, $trailersDeferred, $originalCancellation, $readingCancellation, $bodyCancellationToken, &$backpressure, &$trailers) {
                    ...
                    try {
                        ...
                        if ($timeout > 0 && $parser->getState() !== Http1Parser::BODY_IDENTITY_EOF) {
                            ...
                            $this->estimatedClose = (int) ((\microtime(\true) + $timeout) * 1000);
                        } else {
                            $this->close();
                        }
                        $this->busy = \false;
    ...
    {
        return (int) \max(0, $this->estimatedClose - \microtime(\true) * 1000);
    }
