...
use WP_Ultimo\Dependencies\Amp\Http\Client\Connection\Internal\Http2Stream;
...
use WP_Ultimo\Dependencies\Amp\Http\Client\Trailers;
...
use WP_Ultimo\Dependencies\Amp\Socket\Socket;
...
final class Http2Connection implements Connection
{
    ...
    public const NOFLAG = "\0";
    public const ACK = "\1";
    public const END_STREAM = "\1";
    public const END_HEADERS = "\4";
    public const PADDED = "\10";
    public const PRIORITY_FLAG = " ";
    public const DATA = "\0";
    ...
    private const KNOWN_RESPONSE_PSEUDO_HEADERS = [":status" => \true];
    private const KNOWN_REQUEST_PSEUDO_HEADERS = [":method" => \true, ":authority" => \true, ":path" => \true, ":scheme" => \true];
    ...
    public function getStream(Request $request) : Promise
    {
        ...
        return call(function () use($request) {
            ...
            return new HttpStream($this, \Closure::fromCallable([$this, 'request']), \Closure::fromCallable([$this, 'release']));
        });
    }
    /**
    ...
    private function request(Request $request, CancellationToken $token) : Promise
    {
        ...
        $this->streams[$id] = $stream = new Http2Stream(self::DEFAULT_WINDOW_SIZE, $this->initialWindowSize, $request->getHeaderSizeLimit(), $request->getBodySizeLimit());
        ...
        return call(function () use($id, $request, $token) : \Generator {
            ...
            if ($this->socket->isClosed()) {
                throw new UnprocessedRequestException(new SocketException(\sprintf("Socket to '%s' closed before the request could be sent", $this->socket->getRemoteAddress())));
            }
            ...
            try {
                ...
                $headers = \array_merge([":authority" => [$authority], ":path" => [$path], ":scheme" => [$uri->getScheme()], ":method" => [$request->getMethod()]], $request->getHeaders());
    ...
    private function run() : \Generator
    {
        try {
            ...
            (yield $this->writeFrame(\pack("nNnNnNnNnN", self::ENABLE_PUSH, 1, self::MAX_CONCURRENT_STREAMS, 256, self::INITIAL_WINDOW_SIZE, self::DEFAULT_WINDOW_SIZE, self::MAX_HEADER_LIST_SIZE, self::DEFAULT_MAX_HEADER_SIZE, self::MAX_FRAME_SIZE, self::DEFAULT_MAX_FRAME_SIZE), self::SETTINGS, self::NOFLAG));
            $parser = $this->parser();
            while (null !== ($chunk = (yield $this->socket->read()))) {
                ...
                while ($promise instanceof Promise) {
                    (yield $promise);
                    // Wait for promise to resolve before resuming parser and reading more data.
    ...
    {
        $maxHeaderSize = self::DEFAULT_MAX_FRAME_SIZE;
        ...
        while (\true) {
            ...
            try {
                if ($length > self::DEFAULT_MAX_FRAME_SIZE) {
                    // Do we want to allow increasing max frame size?
                ...
                switch ($type) {
                    case self::DATA:
                        ...
                        if (($flags & self::END_STREAM) !== "\0") {
                            ...
                            if (!isset($this->bodyEmitters[$id], $this->trailerDeferreds[$id])) {
                                continue 2;
                                // Stream closed after emitting body fragment.
                            }
                            $deferred = $this->trailerDeferreds[$id];
                            $emitter = $this->bodyEmitters[$id];
                    ...
                    case self::PUSH_PROMISE:
                        ...
                        $this->streams[$pushedId] = $stream = new Http2Stream(self::DEFAULT_WINDOW_SIZE, 0, $parent->request->getHeaderSizeLimit(), $parent->request->getBodySizeLimit());
                        $stream->parent = $parent;
                        // Set parent stream on new stream.
                        $stream->dependency = $id;
                        $stream->weight = $parent->weight;
                        $stream->cancellationToken = $parent->cancellationToken;
                        $id = $pushedId;
                    // Switch ID to pushed stream for parsing headers.
                ...
                $headers = [];
                $pseudo = [];
                $knownHeaders = $stream->request === null ? self::KNOWN_REQUEST_PSEUDO_HEADERS : self::KNOWN_RESPONSE_PSEUDO_HEADERS;
                ...
                try {
                    ...
                    if ($stream->state & Http2Stream::REMOTE_CLOSED) {
                        ...
                        $this->releaseStream($id);
                        // Response has no body, release stream immediately.
                    } else {
                        ...
                        $tokenSource = new CancellationTokenSource();
                        $cancellationToken = new CombinedCancellationToken($stream->cancellationToken, $tokenSource->getToken());
                        $response = new Response('2', $status, Status::getReason($status), $headers, new ResponseBodyStream(new IteratorStream($this->bodyEmitters[$id]->iterate()), $tokenSource), $stream->request, $this->trailerDeferreds[$id]->promise());
                        $deferred->resolve($response);
                        ...
                        $tokenSource = $cancellationToken = null;
                        // Remove reference to cancellation token.
                    }
                } finally {
                    $deferred = $response = null;
                    // Remove reference to response from parser.
                }
    ...
    private function updateSetting(string $buffer) : void
    {
        ...
        switch ($unpacked["setting"]) {
            ...
            case self::MAX_HEADER_LIST_SIZE:
                return;
            // @TODO Respect these settings from the server.
            default:
                return;
        }
    }
