...
use WP_Ultimo\Dependencies\League\Uri\UriException;
...
final class FollowRedirects implements ApplicationInterceptor
{
    ...
    public function request(Request $request, CancellationToken $cancellation, DelegateHttpClient $next) : Promise
    {
        if ($onPush = $request->getPushCallable()) {
            $request->onPush(function (Request $request, Promise $promise, CancellationTokenSource $source) use($onPush, $cancellation, $next) {
