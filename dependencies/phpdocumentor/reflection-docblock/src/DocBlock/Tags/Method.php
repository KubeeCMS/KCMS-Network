 ...
 */
final class Method extends \WP_Ultimo\Dependencies\phpDocumentor\Reflection\DocBlock\Tags\BaseTag implements \WP_Ultimo\Dependencies\phpDocumentor\Reflection\DocBlock\Tags\Factory\StaticMethod
{
    ...
    /** @var Type */
    private $returnType;
    public function __construct($methodName, array $arguments = [], \WP_Ultimo\Dependencies\phpDocumentor\Reflection\Type $returnType = null, $static = \false, \WP_Ultimo\Dependencies\phpDocumentor\Reflection\DocBlock\Description $description = null)
    {
        \WP_Ultimo\Dependencies\Webmozart\Assert\Assert::stringNotEmpty($methodName);
        \WP_Ultimo\Dependencies\Webmozart\Assert\Assert::boolean($static);
        if ($returnType === null) {
            $returnType = new \WP_Ultimo\Dependencies\phpDocumentor\Reflection\Types\Void_();
        }
        ...
        if (\is_string($arguments) && \strlen($arguments) > 0) {
            ...
            foreach ($arguments as &$argument) {
                ...
                if ($argument[0][0] === '$') {
                    ...
                    $argumentType = new \WP_Ultimo\Dependencies\phpDocumentor\Reflection\Types\Void_();
                } else {
    ...
    private function filterArguments($arguments)
    {
        foreach ($arguments as &$argument) {
            ...
            if (!isset($argument['type'])) {
                $argument['type'] = new \WP_Ultimo\Dependencies\phpDocumentor\Reflection\Types\Void_();
            }
