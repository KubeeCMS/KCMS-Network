...
class LineReader extends \setasign\Fpdi\PdfParser\CrossReference\AbstractReader implements \setasign\Fpdi\PdfParser\CrossReference\ReaderInterface
{
    ...
    protected function extract(\setasign\Fpdi\PdfParser\StreamReader $reader)
    {
        ...
        if ($trailerPos === \false) {
            throw new \setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException('Unexpected end of cross reference. "trailer"-keyword not found.', \setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException::NO_TRAILER_FOUND);
        }
    ...
    protected function read($xrefContent)
    {
        ...
        if (\count($m[0]) === 0) {
            throw new \setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException('No data found in cross-reference.', \setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException::INVALID_DATA);
        }
        ...
        foreach ($lines as $line) {
            ...
            switch (\count($pieces)) {
                ...
                default:
                    throw new \setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException(\sprintf('Unexpected data in xref table (%s)', \implode(' ', $pieces)), \setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException::INVALID_DATA);
            }
        }
