...
class FixedReader extends \setasign\Fpdi\PdfParser\CrossReference\AbstractReader implements \setasign\Fpdi\PdfParser\CrossReference\ReaderInterface
{
    ...
    protected function read()
    {
        ...
        while (($line = $this->reader->readLine(20)) !== \false) {
            ...
            if (!$validityChecked && $entryCount > 0) {
                ...
                if (\strlen(\trim($nextLine)) !== 21) {
                    throw new \setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException('Cross-reference entries are larger than 20 bytes.', \setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException::ENTRIES_TOO_LARGE);
                }
                ...
                if (\strlen(\trim(\substr($nextLine, 0, 20))) !== 18) {
                    throw new \setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException('Cross-reference entries are less than 20 bytes.', \setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException::ENTRIES_TOO_SHORT);
                }
                $validityChecked = \true;
            }
        ...
        if (\count($subSections) === 0) {
            throw new \setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException('No entries found in cross-reference.', \setasign\Fpdi\PdfParser\CrossReference\CrossReferenceException::NO_ENTRIES);
        }
