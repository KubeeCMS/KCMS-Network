    ...
    public function decode($data)
    {
        if ($this->extensionLoaded()) {
            ...
            if ($data === \false) {
                ...
                if (!$data) {
                    throw new \setasign\Fpdi\PdfParser\Filter\FlateException('Error while decompressing stream.', \setasign\Fpdi\PdfParser\Filter\FlateException::DECOMPRESS_ERROR);
                }
            }
        } else {
            throw new \setasign\Fpdi\PdfParser\Filter\FlateException('To handle FlateDecode filter, enable zlib support in PHP.', \setasign\Fpdi\PdfParser\Filter\FlateException::NO_ZLIB);
        }
        return $data;
    }
}
