...
trait FpdiTrait
{
    ...
    protected function getPdfParserInstance(\setasign\Fpdi\PdfParser\StreamReader $streamReader)
    {
        ...
        if (\class_exists(\setasign\FpdiPdfParser\PdfParser\PdfParser::class)) {
            /** @noinspection PhpUndefinedClassInspection */
            return new \setasign\FpdiPdfParser\PdfParser\PdfParser($streamReader);
        }
        return new \setasign\Fpdi\PdfParser\PdfParser($streamReader);
    }
    /**
    ...
    protected function getPdfReaderId($file)
    {
        ...
        if (\is_resource($file)) {
            $streamReader = new \setasign\Fpdi\PdfParser\StreamReader($file);
        } elseif (\is_string($file)) {
            $streamReader = \setasign\Fpdi\PdfParser\StreamReader::createByFile($file);
            $this->createdReaders[] = $id;
        } else {
            $streamReader = $file;
        }
        $reader = new \setasign\Fpdi\PdfReader\PdfReader($this->getPdfParserInstance($streamReader));
        /** @noinspection OffsetOperationsInspection */
     ...
     * @see PageBoundaries
     */
    public function importPage($pageNumber, $box = \setasign\Fpdi\PdfReader\PageBoundaries::CROP_BOX, $groupXObject = \true)
    {
        ...
        $box = \ltrim($box, '/');
        if (!\setasign\Fpdi\PdfReader\PageBoundaries::isValidName($box)) {
        ...
        if ($bbox === \false) {
            throw new \setasign\Fpdi\PdfReader\PdfReaderException(\sprintf("Page doesn't have a boundary box (%s).", $box), \setasign\Fpdi\PdfReader\PdfReaderException::MISSING_DATA);
        }
        $dict = new \setasign\Fpdi\PdfParser\Type\PdfDictionary();
        $dict->value['Type'] = \setasign\Fpdi\PdfParser\Type\PdfName::create('XObject');
        $dict->value['Subtype'] = \setasign\Fpdi\PdfParser\Type\PdfName::create('Form');
        $dict->value['FormType'] = \setasign\Fpdi\PdfParser\Type\PdfNumeric::create(1);
        $dict->value['BBox'] = $bbox->toPdfArray();
        if ($groupXObject) {
            $this->setMinPdfVersion('1.4');
            $dict->value['Group'] = \setasign\Fpdi\PdfParser\Type\PdfDictionary::create(['Type' => \setasign\Fpdi\PdfParser\Type\PdfName::create('Group'), 'S' => \setasign\Fpdi\PdfParser\Type\PdfName::create('Transparency')]);
        }
        ...
        if ($a != 1 || $b != 0 || $c != 0 || $d != 1 || $e != 0 || $f != 0) {
            $dict->value['Matrix'] = \setasign\Fpdi\PdfParser\Type\PdfArray::create([\setasign\Fpdi\PdfParser\Type\PdfNumeric::create($a), \setasign\Fpdi\PdfParser\Type\PdfNumeric::create($b), \setasign\Fpdi\PdfParser\Type\PdfNumeric::create($c), \setasign\Fpdi\PdfParser\Type\PdfNumeric::create($d), \setasign\Fpdi\PdfParser\Type\PdfNumeric::create($e), \setasign\Fpdi\PdfParser\Type\PdfNumeric::create($f)]);
        }
        // try to use the existing content stream
        $pageDict = $page->getPageDictionary();
        $contentsObject = \setasign\Fpdi\PdfParser\Type\PdfType::resolve(\setasign\Fpdi\PdfParser\Type\PdfDictionary::get($pageDict, 'Contents'), $reader->getParser(), \true);
        $contents = \setasign\Fpdi\PdfParser\Type\PdfType::resolve($contentsObject, $reader->getParser());
        // just copy the stream reference if it is only a single stream
        if (($contentsIsStream = $contents instanceof \setasign\Fpdi\PdfParser\Type\PdfStream) || $contents instanceof \setasign\Fpdi\PdfParser\Type\PdfArray && \count($contents->value) === 1) {
            if ($contentsIsStream) {
                /**
                ...
                $stream = \setasign\Fpdi\PdfParser\Type\PdfType::resolve($contents->value[0], $reader->getParser());
            }
            $filter = \setasign\Fpdi\PdfParser\Type\PdfDictionary::get($stream->value, 'Filter');
            if (!$filter instanceof \setasign\Fpdi\PdfParser\Type\PdfNull) {
                $dict->value['Filter'] = $filter;
            }
            $length = \setasign\Fpdi\PdfParser\Type\PdfType::resolve(\setasign\Fpdi\PdfParser\Type\PdfDictionary::get($stream->value, 'Length'), $reader->getParser());
            ...
            $dict->value['Length'] = \setasign\Fpdi\PdfParser\Type\PdfNumeric::create(\strlen($streamContent));
            if ($this->compress) {
                $dict->value['Filter'] = \setasign\Fpdi\PdfParser\Type\PdfName::create('FlateDecode');
            }
            $stream = \setasign\Fpdi\PdfParser\Type\PdfStream::create($dict, $streamContent);
        }
    ...
    {
        if ($value instanceof \setasign\Fpdi\PdfParser\Type\PdfNumeric) {
            if (\is_int($value->value)) {
                $this->_put($value->value . ' ', \false);
            } else {
                $this->_put(\rtrim(\rtrim(\sprintf('%.5F', $value->value), '0'), '.') . ' ', \false);
            }
        } elseif ($value instanceof \setasign\Fpdi\PdfParser\Type\PdfName) {
            $this->_put('/' . $value->value . ' ', \false);
        } elseif ($value instanceof \setasign\Fpdi\PdfParser\Type\PdfString) {
            $this->_put('(' . $value->value . ')', \false);
        } elseif ($value instanceof \setasign\Fpdi\PdfParser\Type\PdfHexString) {
            $this->_put('<' . $value->value . '>');
        } elseif ($value instanceof \setasign\Fpdi\PdfParser\Type\PdfBoolean) {
            $this->_put($value->value ? 'true ' : 'false ', \false);
        } elseif ($value instanceof \setasign\Fpdi\PdfParser\Type\PdfArray) {
            $this->_put('[', \false);
            foreach ($value->value as $entry) {
                $this->writePdfType($entry);
            }
            $this->_put(']');
        } elseif ($value instanceof \setasign\Fpdi\PdfParser\Type\PdfDictionary) {
            $this->_put('<<', \false);
            ...
            $this->_put('>>');
        } elseif ($value instanceof \setasign\Fpdi\PdfParser\Type\PdfToken) {
            $this->_put($value->value);
        } elseif ($value instanceof \setasign\Fpdi\PdfParser\Type\PdfNull) {
            $this->_put('null ');
            ...
            $this->_put('endstream');
        } elseif ($value instanceof \setasign\Fpdi\PdfParser\Type\PdfIndirectObjectReference) {
