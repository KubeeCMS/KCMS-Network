...
class Base
{
    ...
    public static function regexify($regex = '')
    {
        ...
        $regex = \preg_replace_callback('/(\\[[^\\]]+\\])\\{(\\d+),(\\d+)\\}/', function ($matches) {
            return \str_repeat($matches[1], \Faker\Provider\Base::randomElement(\range($matches[2], $matches[3])));
        }, $regex);
        ...
        $regex = \preg_replace_callback('/\\((.*?)\\)/', function ($matches) {
            return \Faker\Provider\Base::randomElement(\explode('|', \str_replace(array('(', ')'), '', $matches[1])));
        }, $regex);
        ...
        $regex = \preg_replace_callback('/\\[([^\\]]+)\\]/', function ($matches) {
            return \Faker\Provider\Base::randomElement(\str_split($matches[1]));
        }, $regex);
    ...
    public function optional($weight = 0.5, $default = null)
    {
        ...
        return new \Faker\DefaultGenerator($default);
    }
    /**
    ...
    public function unique($reset = \false, $maxRetries = 10000)
    {
        if ($reset || !$this->unique) {
            $this->unique = new \Faker\UniqueGenerator($this->generator, $maxRetries);
        }
        return $this->unique;
    }
    /**
    ...
    {
        return new \Faker\ValidGenerator($this->generator, $validator, $maxRetries);
    }
}
