...
use WP_Ultimo\Dependencies\Doctrine\Common\Persistence\ObjectManager;
use WP_Ultimo\Dependencies\Doctrine\Common\Persistence\Mapping\ClassMetadata;
/**
...
class EntityPopulator
{
    ...
    public function guessColumnFormatters(\Faker\Generator $generator)
    {
        ...
        $columnTypeGuesser = new \Faker\ORM\Doctrine\ColumnTypeGuesser($generator);
        ...
        foreach ($this->class->getAssociationNames() as $assocName) {
            ...
            if ($this->class instanceof \WP_Ultimo\Dependencies\Doctrine\ORM\Mapping\ClassMetadata) {
                $mappings = $this->class->getAssociationMappings();
                foreach ($mappings as $mapping) {
                    if ($mapping['targetEntity'] == $relatedClass) {
                        if ($mapping['type'] == \WP_Ultimo\Dependencies\Doctrine\ORM\Mapping\ClassMetadata::ONE_TO_ONE) {
                            $unique = \true;
            ...
            } elseif ($this->class instanceof \WP_Ultimo\Dependencies\Doctrine\ODM\MongoDB\Mapping\ClassMetadata) {
                $mappings = $this->class->associationMappings;
                foreach ($mappings as $mapping) {
                    if ($mapping['targetDocument'] == $relatedClass) {
                        if ($mapping['type'] == \WP_Ultimo\Dependencies\Doctrine\ODM\MongoDB\Mapping\ClassMetadata::ONE && $mapping['association'] == \WP_Ultimo\Dependencies\Doctrine\ODM\MongoDB\Mapping\ClassMetadata::REFERENCE_ONE) {
                            $unique = \true;
