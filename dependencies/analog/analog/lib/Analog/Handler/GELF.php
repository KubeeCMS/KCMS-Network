...
class GELF
{
    public static function init($host = '127.0.0.1', $port = \WP_Ultimo\Dependencies\GELFMessagePublisher::GRAYLOG2_DEFAULT_PORT)
    {
        $publisher = new \WP_Ultimo\Dependencies\GELFMessagePublisher($host, $port);
        return function ($info) use($publisher) {
            $message = new \WP_Ultimo\Dependencies\GELFMessage();
