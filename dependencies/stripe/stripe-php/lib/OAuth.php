...
abstract class OAuth
{
    ...
    {
        $params = $params ?: [];
        $base = $opts && \array_key_exists('connect_base', $opts) ? $opts['connect_base'] : \WP_Ultimo\Dependencies\Stripe\Stripe::$connectBase;
    ...
    {
        $base = $opts && \array_key_exists('connect_base', $opts) ? $opts['connect_base'] : \WP_Ultimo\Dependencies\Stripe\Stripe::$connectBase;
        $requestor = new \WP_Ultimo\Dependencies\Stripe\ApiRequestor(null, $base);
        list($response, $apiKey) = $requestor->request('post', '/oauth/token', $params, null);
        return \WP_Ultimo\Dependencies\Stripe\Util\Util::convertToStripeObject($response->json, $opts);
    }
    /**
    ...
    private static function _getClientId($params = null)
    {
        ...
        if (null === $clientId) {
            $clientId = \WP_Ultimo\Dependencies\Stripe\Stripe::getClientId();
        }
        if (null === $clientId) {
