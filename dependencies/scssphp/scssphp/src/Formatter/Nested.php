 ...
 */
class Nested extends \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter
{
    /**
     * @var integer
     */
    ...
    protected function block(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\OutputBlock $block)
    {
        ...
        $isMediaOrDirective = \in_array($block->type, [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_DIRECTIVE, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MEDIA]);
        $isSupport = $block->type === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_DIRECTIVE && $block->selectors && \strpos(\implode('', $block->selectors), '@supports') !== \false;
        while ($block->depth < \end($depths) || $block->depth == 1 && \end($depths) == 1) {
            \array_pop($depths);
            $this->depth--;
            if (!$this->depth && ($block->depth <= 1 || !$this->indentLevel && $block->type === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COMMENT) && ($block->selectors && !$isMediaOrDirective || $previousHasSelector)) {
        ...
        $previousEmpty = $block->type === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COMMENT;
        $previousHasSelector = \false;
