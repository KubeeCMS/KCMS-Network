...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block\AtRootBlock;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block\CallableBlock;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block\DirectiveBlock;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block\EachBlock;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block\ElseBlock;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block\ElseifBlock;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block\ForBlock;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block\IfBlock;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block\MediaBlock;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block\NestedPropertyBlock;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block\WhileBlock;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Compiler\CachedResult;
...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Exception\SassException;
...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Logger\StreamLogger;
...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Util\Path;
/**
...
final class Compiler
{
     ...
     */
    private static $operatorNames = ['+' => 'add', '-' => 'sub', '*' => 'mul', '/' => 'div', '%' => 'mod', '==' => 'eq', '!=' => 'neq', '<' => 'lt', '>' => 'gt', '<=' => 'lte', '>=' => 'gte'];
    /**
     * @var array<string, string>
     */
    private static $namespaces = ['special' => '%', 'mixin' => '@', 'function' => '^'];
     ...
     */
    private $registeredFeatures = ['extend-selector-pseudoclass' => \false, 'at-error' => \true, 'units-level-3' => \true, 'global-variable-shadowing' => \false];
    /**
     * @var int
    ...
    {
        $options = ['importPaths' => $this->importPaths, 'registeredVars' => $this->registeredVars, 'sourceMap' => \serialize($this->sourceMap), 'sourceMapOptions' => $this->sourceMapOptions, 'outputStyle' => $this->outputStyle];
        return $options;
    }
    /**
     ...
     * @return void
     */
    private function matchExtends(array $selector, &$out, int $from = 0, bool $initial = \true) : void
    {
        static $partsPile = [];
     ...
     * @return bool
     */
    private function matchExtendsSingle(array $rawSingle, &$outOrigin, bool $initial = \true) : bool
    {
        $counts = [];
        $single = [];
        ...
        switch ($child[0]) {
            ...
            case Type::T_ASSIGN:
                ...
                if ($name[0] === Type::T_VARIABLE) {
                    ...
                    $shouldSet = $isDefault && (\is_null($result = $this->get($name[1], \false)) || $result === self::$null);
            ...
            case Type::T_FUNCTION:
                ...
                $this->set(self::$namespaces[$block->type] . $block->name, $block, \true);
                break;
            case Type::T_EXTEND:
...
EOL;
                ...
                foreach ($list[2] as $item) {
                    if (\count($each->vars) === 1) {
                        ...
                        foreach ($each->vars as $i => $var) {
                            $this->set($var, isset($values[$i]) ? $values[$i] : self::$null, \true);
                        }
                    }
    ...
    {
        $truthy = $left === self::$null || $right === self::$null || ($left === self::$false || $left === self::$true) && ($right === self::$false || $right === self::$true);
        if (!$shouldEval) {
            if (!$truthy) {
                return null;
            }
     ...
     * @return string
     */
    public function compileValue($value, bool $quote = \true) : string
    {
        $value = $this->reduce($value);
        if ($value instanceof Number) {
     ...
     * @return string
     */
    private function compileStringContent($string, bool $quote = \true) : string
    {
        $parts = [];
     ...
     * @return mixed|null
     */
    private function get(string $name, bool $shouldThrow = \true, ?Environment $env = null, bool $unreduced = \false)
    {
        $normalizedName = $this->normalizeName($name);
        $specialContentKey = self::$namespaces['special'] . 'content';
        if (!isset($env)) {
     ...
     */
    private function evaluateArguments(array $args, bool $reduce = \true) : array
    {
     ...
     * @throws \Exception
     */
    private function applyArguments(array $argDef, ?array $argValues, bool $storeInEnv = \true, bool $reduce = \true)
    {
        $output = [];
     ...
     * @return int|mixed
     */
    private function compileColorPartValue($value, $min, $max, bool $isInt = \true)
    {
        if (!\is_numeric($value)) {
            if (\is_array($value)) {
    ...
    private static $libGetFunction = [['name'], ['name', 'css']];
    private function libGetFunction($args)
    {
        ...
        if (\count($args)) {
            ...
            $isCss = $isCss === self::$true ? \true : \false;
        }
        if ($isCss) {
    ...
    private static $libRgb = [['color'], ['color', 'alpha'], ['channels'], ['red', 'green', 'blue'], ['red', 'green', 'blue', 'alpha']];
    /**
     * @param array $args
     * @param array $kwargs
    ...
    private static $libRgba = [['color'], ['color', 'alpha'], ['channels'], ['red', 'green', 'blue'], ['red', 'green', 'blue', 'alpha']];
    ...
    // mix two colors
    private static $libMix = [['color1', 'color2', 'weight:50%'], ['color-1', 'color-2', 'weight:50%']];
    ...
    private static $libHsl = [['channels'], ['hue', 'saturation'], ['hue', 'saturation', 'lightness'], ['hue', 'saturation', 'lightness', 'alpha']];
    /**
     * @param array $args
     * @param array $kwargs
     * @param string $funcName
    ...
    private static $libHsla = [['channels'], ['hue', 'saturation'], ['hue', 'saturation', 'lightness'], ['hue', 'saturation', 'lightness', 'alpha']];
    ...
    */
    ...
    private static $libMapRemove = [['map'], ['map', 'key', 'keys...']];
    ...
    private static $libMapMerge = [['map1', 'map2'], ['map-1', 'map-2'], ['map1', 'args...']];
    ...
    private static $libComparable = [['number1', 'number2'], ['number-1', 'number-2']];
    ...
    private static $libSelectorExtend = [['selector', 'extendee', 'extender'], ['selectors', 'extendee', 'extender']];
    ...
    private static $libSelectorReplace = [['selector', 'original', 'replacement'], ['selectors', 'original', 'replacement']];
    ...
    private static $libSelectorParse = [['selector'], ['selectors']];
