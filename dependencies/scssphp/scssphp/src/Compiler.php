...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\Compressed;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\Expanded;
...
class Compiler
{
    ...
    public static $true = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, 'true'];
    public static $false = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, 'false'];
    /** @deprecated */
    public static $NaN = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, 'NaN'];
    /** @deprecated */
    public static $Infinity = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, 'Infinity'];
    public static $null = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NULL];
    public static $nullString = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', []];
    public static $defaultValue = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, ''];
    public static $selfSelector = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_SELF];
    public static $emptyList = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, '', []];
    public static $emptyMap = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MAP, [], []];
    public static $emptyString = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '"', []];
    public static $with = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, 'with'];
    public static $without = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, 'without'];
    /**
     ...
     */
    protected $formatter = \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\Expanded::class;
    /**
     * @var Environment
     */
    protected $rootEnv;
    ...
    public function __construct($cacheOptions = null)
    {
        ...
        $this->sourceNames = [];
        if ($cacheOptions) {
            $this->cache = new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Cache($cacheOptions);
        }
    ...
    public function compile($code, $path = null)
    {
        ...
        try {
            ...
            if ($this->sourceMap) {
                if (\is_object($this->sourceMap) && $this->sourceMap instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\SourceMap\SourceMapGenerator) {
                    $sourceMapGenerator = $this->sourceMap;
                    $this->sourceMap = self::SOURCE_MAP_FILE;
                } elseif ($this->sourceMap !== self::SOURCE_MAP_NONE) {
                    $sourceMapGenerator = new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\SourceMap\SourceMapGenerator($this->sourceMapOptions);
                }
            }
            $out = $this->formatter->format($this->scope, $sourceMapGenerator);
            $prefix = '';
            if (!$this->charsetSeen) {
                if (\strlen($out) !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Util::mbStrlen($out)) {
            ...
            if (!empty($out) && $this->sourceMap && $this->sourceMap !== self::SOURCE_MAP_NONE) {
                ...
                switch ($this->sourceMap) {
                    case self::SOURCE_MAP_INLINE:
                        $sourceMapUrl = \sprintf('data:application/json,%s', \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Util::encodeURIComponent($sourceMap));
                        break;
    ...
    protected function parserFactory($path)
    {
        ...
        $parser = new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Parser($path, \count($this->sourceNames), $this->encoding, $this->cache, $cssOnly);
    ...
    {
        $out = new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\OutputBlock();
        $out->type = $type;
        $out->lines = [];
        $out->children = [];
        ...
        if ($this->env->block instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block) {
     ...
     * @return void
     */
    protected function compileRoot(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block $rootBlock)
    {
        $this->rootBlock = $this->scope = $this->makeOutputBlock(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_ROOT);
    ...
    protected function missingSelectors()
    {
        foreach ($this->extends as $extend) {
            ...
            $this->sourceLine = $block[\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Parser::SOURCE_LINE];
     ...
     * @return void
     */
    protected function flattenSelectors(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\OutputBlock $block, $parentKey = null)
    {
        if ($block->selectors) {
            $selectors = [];
     ...
     * @return void
     */
    protected function compileMedia(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block $media)
    {
        $this->pushEnv($media);
        ...
        if (!empty($mediaQueries) && $mediaQueries) {
            ...
            foreach ($mediaQueries as $mediaQuery) {
                $this->scope = $this->makeOutputBlock(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MEDIA, [$mediaQuery]);
            ...
            foreach ($media->children as $child) {
                $type = $child[0];
                if ($type !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_BLOCK && $type !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MEDIA && $type !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_DIRECTIVE && $type !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_IMPORT) {
                    $needsWrap = \true;
                    break;
                }
            }
            if ($needsWrap) {
                $wrapped = new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block();
                ...
                $media->children = [[\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_BLOCK, $wrapped]];
            }
     ...
     * @return \ScssPhp\ScssPhp\Formatter\OutputBlock
     */
    protected function mediaParent(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\OutputBlock $scope)
    {
        while (!empty($scope->parent)) {
            if (!empty($scope->type) && $scope->type !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MEDIA) {
                break;
            }
            $scope = $scope->parent;
        }
     ...
     * @return void
     */
    protected function compileDirective($directive, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\OutputBlock $out)
    {
        if (\is_array($directive)) {
            ...
            if (empty($directive[3])) {
                $this->appendRootDirective($s . ';' . $appendNewLine, $out, [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COMMENT, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_DIRECTIVE]);
            } else {
                $this->appendOutputLine($out, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_DIRECTIVE, $s . ';');
            }
        } else {
     ...
     * @return void
     */
    protected function compileAtRoot(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block $block)
    {
        ...
        if ($block->selector) {
            ...
            $block->children = [[\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_BLOCK, $wrapped]];
            $block->selector = null;
        }
    ...
    protected function filterScopeWithWithout($scope, $with, $without)
    {
        $filteredScopes = [];
        $childStash = [];
        if ($scope->type === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\TYPE::T_ROOT) {
            return $scope;
        }
        // start from the root
        while ($scope->parent && $scope->parent->type !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\TYPE::T_ROOT) {
        ...
        for (;;) {
            ...
            if ($this->isWith($scope, $with, $without)) {
                ...
                $s->lines = [];
                $s->parent = null;
                if ($s->type !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MEDIA && $s->type !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_DIRECTIVE) {
                    $s->selectors = [];
                }
                $filteredScopes[] = $s;
            }
    ...
    protected function compileWith($withCondition)
    {
        ...
        $without = ['rule' => \true];
        if ($withCondition) {
            if ($withCondition[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_INTERPOLATE) {
    ...
    {
        if (isset($block->type)) {
            if ($block->type === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MEDIA) {
                return $this->testWithWithout('media', $with, $without);
            }
            if ($block->type === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_DIRECTIVE) {
                if (isset($block->name)) {
            ...
            if (\count($block->selectors)) {
                ...
                if (\is_object($s) && $s instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number) {
     ...
     * @return void
     */
    protected function compileKeyframeBlock(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block $block, $selectors)
    {
        $env = $this->pushEnv($block);
        $envs = $this->compactEnv($env);
        $this->env = $this->extractEnv(\array_filter($envs, function (\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Compiler\Environment $e) {
     ...
     * @return void
     */
    protected function compileNestedPropertiesBlock(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block $block, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\OutputBlock $out)
    {
        ...
        foreach ($block->children as $child) {
            switch ($child[0]) {
                case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_ASSIGN:
                    \array_unshift($child[1][2], $prefix);
                    break;
                case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NESTED_PROPERTY:
     ...
     * @return void
     */
    protected function compileNestedBlock(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block $block, $selectors)
    {
        $this->pushEnv($block);
        $this->scope = $this->makeOutputBlock($block->type, $selectors);
        $this->scope->parent->children[] = $this->scope;
        ...
        if ($block->type !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_DIRECTIVE || $this->compileDirectiveName($block->name) !== 'font-face') {
            // need wrapping?
            $needWrapping = \false;
            foreach ($block->children as $child) {
                if ($child[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_ASSIGN) {
                    $needWrapping = \true;
                    break;
                }
            }
     ...
     * @return void
     */
    protected function compileBlock(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block $block)
    {
        $env = $this->pushEnv($block);
    ...
    {
        $out = $this->makeOutputBlock(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COMMENT);
    ...
    protected function evalSelectors($selectors)
    {
        ...
        if ($this->shouldEvaluate) {
            ...
            $parser = $this->parserFactory(__METHOD__);
            if ($parser->parseSelector($buffer, $newSelectors, \true)) {
    ...
    {
        foreach ($part as &$p) {
            if (\is_array($p) && ($p[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_INTERPOLATE || $p[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING)) {
    ...
    protected function collapseSelectors($selectors, $selectorFormat = \false)
    {
        ...
        foreach ($selectors as $selector) {
            ...
            if ($selectorFormat) {
                foreach ($output as &$o) {
                    $o = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', [$o]];
                }
                $output = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, ' ', $output];
            } else {
        ...
        if ($selectorFormat) {
            $parts = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, ',', $parts];
        } else {
            $parts = \implode(', ', $parts);
        }
        return $parts;
    ...
    protected function replaceSelfSelector($selectors, $replace = null)
    {
        foreach ($selectors as &$part) {
            if (\is_array($part)) {
                if ($part === [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_SELF]) {
                    if (\is_null($replace)) {
                        $replace = $this->reduce([\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_SELF]);
    ...
    protected function compileSelectorPart($piece)
    {
        foreach ($piece as &$p) {
            ...
            switch ($p[0]) {
                case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_SELF:
                    $p = '&';
                    break;
                default:
                    $p = $this->compileValue($p);
    ...
    {
        $this->callStack[] = ['n' => $name, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Parser::SOURCE_INDEX => $this->sourceIndex, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Parser::SOURCE_LINE => $this->sourceLine, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Parser::SOURCE_COLUMN => $this->sourceColumn];
     ...
     * @return array|null
     */
    protected function compileChildren($stms, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\OutputBlock $out, $traceName = '')
    {
     ...
     * @throws \Exception
     */
    protected function compileChildrenNoReturn($stms, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\OutputBlock $out, $selfParent = null, $traceName = '')
    {
        $this->pushCallStack($traceName);
        foreach ($stms as $stm) {
            if ($selfParent && isset($stm[1]) && \is_object($stm[1]) && $stm[1] instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block) {
                $stm[1]->selfParent = $selfParent;
                $ret = $this->compileChild($stm, $out);
                $stm[1]->selfParent = null;
            } elseif ($selfParent && \in_array($stm[0], [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\TYPE::T_INCLUDE, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\TYPE::T_EXTEND])) {
        ...
        foreach ($queryList as $kql => $query) {
            ...
            foreach ($query as $kq => $q) {
                for ($i = 1; $i < \count($q); $i++) {
                    ...
                    if ($q[0] == \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MEDIA_TYPE && (\strpos($value, '(') !== \false || \strpos($value, ')') !== \false || \strpos($value, ':') !== \false || \strpos($value, ',') !== \false)) {
                        $shouldReparse = \true;
                    }
                    $queryList[$kql][$kq][$i] = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, $value];
                }
            }
            if ($shouldReparse) {
                if (\is_null($parser)) {
    ...
    protected function compileMediaQuery($queryList)
    {
        ...
        foreach ($queryList as $query) {
            ...
            $mediaTypeOnly = \true;
            foreach ($query as $q) {
                if ($q[0] !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MEDIA_TYPE) {
                    $mediaTypeOnly = \false;
                    break;
                }
            }
            foreach ($query as $q) {
                switch ($q[0]) {
                    case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MEDIA_TYPE:
                        ...
                        if (!$mediaTypeOnly) {
                            if (\in_array(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NOT, $newType) || $type && \in_array(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NOT, $type)) {
                                if ($type) {
                        ...
                        break;
                    case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MEDIA_EXPRESSION:
                        if (isset($q[2])) {
                        ...
                        break;
                    case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MEDIA_VALUE:
                        $parts[] = $this->compileValue($q[1]);
                        break;
                }
            }
    ...
    protected function mergeMediaTypes($type1, $type2)
    {
        ...
        if ($m1 === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NOT ^ $m2 === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NOT) {
            if ($t1 === $t2) {
                return null;
            }
            return [$m1 === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NOT ? $m2 : $m1, $m1 === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NOT ? $t2 : $t1];
        }
        if ($m1 === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NOT && $m2 === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NOT) {
            ...
            return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NOT, $t1];
        }
        if ($t1 !== $t2) {
            return null;
        }
     ...
     * @return boolean
     */
    protected function compileImport($rawPath, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\OutputBlock $out, $once = \false)
    {
        if ($rawPath[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING) {
        ...
        if ($rawPath[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST) {
            // handle a list of strings
            if (\count($rawPath[2]) === 0) {
                return \false;
            }
            foreach ($rawPath[2] as $path) {
                if ($path[0] !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING) {
                    $this->appendRootDirective('@import ' . $this->compileImportPath($rawPath) . ';', $out);
                    return \false;
                }
            }
    ...
    {
        switch ($path[0]) {
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST:
                foreach ($path[2] as $k => $v) {
                    $path[2][$k] = $this->escapeImportPathString($v);
                }
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING:
                if ($path[1]) {
                    $path = $this->compileValue($path);
                    $path = \str_replace(' ', '\\ ', $path);
                    $path = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, $path];
                }
                break;
        }
        return $path;
     ...
     * @return void
     */
    protected function appendRootDirective($line, $out, $allowed = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COMMENT])
    {
        $root = $out;
        while ($root->parent) {
            $root = $root->parent;
        ...
        $child = $this->makeOutputBlock(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COMMENT);
        $child->lines[] = $line;
     ...
     * @return void
     */
    protected function appendOutputLine(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\OutputBlock $out, $type, $line)
    {
        $outWrite =& $out;
     ...
     */
    protected function compileChild($child, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\OutputBlock $out)
    {
        if (isset($child[\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Parser::SOURCE_LINE])) {
            $this->sourceIndex = isset($child[\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Parser::SOURCE_INDEX]) ? $child[\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Parser::SOURCE_INDEX] : null;
            $this->sourceLine = isset($child[\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Parser::SOURCE_LINE]) ? $child[\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Parser::SOURCE_LINE] : -1;
            $this->sourceColumn = isset($child[\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Parser::SOURCE_COLUMN]) ? $child[\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Parser::SOURCE_COLUMN] : -1;
        ...
        switch ($child[0]) {
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_SCSSPHP_IMPORT_ONCE:
                $rawPath = $this->reduce($child[1]);
                $this->compileImport($rawPath, $out, \true);
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_IMPORT:
                $rawPath = $this->reduce($child[1]);
                $this->compileImport($rawPath, $out);
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_DIRECTIVE:
                $this->compileDirective($child[1], $out);
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_AT_ROOT:
                $this->compileAtRoot($child[1]);
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MEDIA:
                $this->compileMedia($child[1]);
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_BLOCK:
                $this->compileBlock($child[1]);
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_CHARSET:
                if (!$this->charsetSeen) {
                    $this->charsetSeen = \true;
                    $this->appendRootDirective('@charset ' . $this->compileValue($child[1]) . ';', $out);
                }
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_CUSTOM_PROPERTY:
                list(, $name, $value) = $child;
                $compiledName = $this->compileValue($name);
                // if the value reduces to null from something else then
                // the property should be discarded
                if ($value[0] !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NULL) {
                    $value = $this->reduce($value);
                    if ($value[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NULL || $value === static::$nullString) {
                        break;
                    }
                }
                $compiledValue = $this->compileValue($value);
                $line = $this->formatter->customProperty($compiledName, $compiledValue);
                $this->appendOutputLine($out, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_ASSIGN, $line);
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_ASSIGN:
                list(, $name, $value) = $child;
                if ($name[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_VARIABLE) {
                ...
                if (\in_array($compiledName, ['font', 'grid-row', 'grid-column', 'border-radius'])) {
                    if ($value[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_VARIABLE) {
                    ...
                    if ($compiledName === 'font' && $value[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST && $value[1] === ',') {
                    ...
                    if ($shorthandValue[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_EXPRESSION && $shorthandValue[1] === '/') {
                        $revert = \true;
                        if ($shorthandDividerNeedsUnit) {
                            ...
                            if ($divider instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number && \intval($divider->getDimension()) && $divider->unitless()) {
                                $revert = \false;
                            }
                        }
                        if ($revert) {
                    ...
                    } elseif ($shorthandValue[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST) {
                        foreach ($shorthandValue[2] as &$item) {
                            if ($item[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_EXPRESSION && $item[1] === '/') {
                ...
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COMMENT:
                if ($out->type === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_ROOT) {
                    $this->compileComment($child);
                    break;
                }
                $line = $this->compileCommentValue($child, \true);
                $this->appendOutputLine($out, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COMMENT, $line);
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MIXIN:
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION:
                list(, $block) = $child;
                ...
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_EXTEND:
                ...
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_IF:
                list(, $if) = $child;
                if ($this->isTruthy($this->reduce($if->cond, \true))) {
                    return $this->compileChildren($if->children, $out);
                }
                foreach ($if->cases as $case) {
                    if ($case->type === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_ELSE || $case->type === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_ELSEIF && $this->isTruthy($this->reduce($case->cond))) {
                        return $this->compileChildren($case->children, $out);
                    }
                }
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_EACH:
                list(, $each) = $child;
                ...
                $this->backPropagateEnv($store, $each->vars);
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_WHILE:
                list(, $while) = $child;
                ...
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FOR:
                list(, $for) = $child;
                $start = $this->reduce($for->start, \true);
                $end = $this->reduce($for->end, \true);
                if (!$start instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number) {
                    throw $this->error('%s is not a number', $start[0]);
                }
                if (!$end instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number) {
                ...
                for (;;) {
                    ...
                    $this->set($for->var, new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number($start, $numeratorUnits, $denominatorUnits));
                    $start += $d;
                ...
                $this->backPropagateEnv($store, [$for->var]);
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_RETURN:
                return $this->reduce($child[1], \true);
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NESTED_PROPERTY:
                $this->compileNestedPropertiesBlock($child[1], $out);
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_INCLUDE:
                // including a mixin
                ...
                if (isset($child['selfParent']) && isset($child['selfParent']->selectors)) {
                    ...
                    if ($parentSelectors) {
                        $parent = new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block();
                        $parent->selectors = $parentSelectors;
                        foreach ($mixin->children as $k => $child) {
                            if (isset($child[1]) && \is_object($child[1]) && $child[1] instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block) {
                ...
                $this->popEnv();
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MIXIN_CONTENT:
                ...
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_DEBUG:
                list(, $value) = $child;
                ...
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_WARN:
                list(, $value) = $child;
                $fname = $this->getPrettyPath($this->sourceNames[$this->sourceIndex]);
                $line = $this->sourceLine;
                $value = $this->compileDebugValue($value);
                ...
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_ERROR:
                list(, $value) = $child;
                $fname = $this->getPrettyPath($this->sourceNames[$this->sourceIndex]);
                $line = $this->sourceLine;
    ...
    protected function expToString($exp, $keepParens = \false)
    {
        ...
        return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', $content];
    }
    /**
     * Is truthy?
     *
    ...
    {
        switch ($value[0]) {
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_EXPRESSION:
                if ($value[1] === '/') {
                    return $this->shouldEval($value[2]) || $this->shouldEval($value[3]);
                }
            // fall-thru
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_VARIABLE:
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION_CALL:
                return \true;
        }
        return \false;
    }
    ...
    protected function reduce($value, $inExp = \false)
    {
        ...
        switch ($value[0]) {
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_EXPRESSION:
                ...
                if ($opName == 'div' && !$inParens && !$inExp && ($right[0] !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NUMBER && isset($right[2]) && $right[2] != '' || $right[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NUMBER && !$right->unitless())) {
                ...
                return $this->expToString($value);
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_UNARY:
                list(, $op, $exp, $inParens) = $value;
                $inExp = $inExp || $this->shouldEval($exp);
                $exp = $this->reduce($exp);
                if ($exp instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number) {
                    switch ($op) {
                        case '+':
                            return $exp;
                        case '-':
                ...
                return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', [$op, $exp]];
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_VARIABLE:
            ...
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST:
                ...
                return $value;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MAP:
            ...
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING:
                ...
                return $value;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_INTERPOLATE:
    ...
    protected function fncall($functionReference, $argValues)
    {
        ...
        if (\is_string($functionReference)) {
            ...
            if ($functionReference === static::$null || $functionReference[0] !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION_REFERENCE) {
                $functionReference = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION, $name, [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, ',', []]];
            }
        }
        // a function type means we just want a plain css function call
        if ($functionReference[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION) {
            ...
            return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION, $functionReference[1], [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, ',', $listArgs]];
        }
        if ($functionReference === static::$null || $functionReference[0] !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION_REFERENCE) {
        ...
        switch ($functionReference[1]) {
            ...
            case 'native':
                ...
                if (\in_array($name, ['min', 'max'])) {
                    $cssFunction = $this->cssValidArg([\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION_CALL, $name, $argValues], ['min', 'max', 'calc', 'env', 'var']);
                ...
                if (!isset($returnValue)) {
                    return $this->fncall([\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION, $name, [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, ',', []]], $argValues);
                }
                return $returnValue;
            default:
                return static::$defaultValue;
    ...
    protected function cssValidArg($arg, $allowed_function = [], $inFunction = \false)
    {
        switch ($arg[0]) {
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_INTERPOLATE:
                return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, $this->CompileValue($arg)];
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION:
                if (!\in_array($arg[1], $allowed_function)) {
                    return \false;
                }
                if ($arg[2][0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST) {
            ...
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION_CALL:
                ...
                return $this->fncall([\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION, $arg[1], [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, ',', []]], $cssArgs);
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING:
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD:
                if (!$inFunction or !\in_array($inFunction, ['calc', 'env', 'var'])) {
                    return \false;
                }
                return $this->stringifyFncallArgs($arg);
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NUMBER:
                return $this->stringifyFncallArgs($arg);
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST:
                if (!$inFunction) {
                    return \false;
                ...
                if (empty($arg['enclosing']) and $arg[1] === '') {
                    ...
                    $arg[0] = \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING;
                    return $arg;
                }
                return \false;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_EXPRESSION:
        ...
        switch ($arg[0]) {
            ...
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION_CALL:
                ...
                if (\in_array($name, ['max', 'min', 'calc'])) {
                    $args = $arg[2];
                    $arg = $this->fncall([\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION, $name, [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, ',', []]], $args);
                }
                break;
        }
        return $arg;
    ...
    protected function getFunctionReference($name, $safeCopy = \false)
    {
        ...
        if ($func = $this->get(static::$namespaces['function'] . $name, \false)) {
            ...
            return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION_REFERENCE, 'scss', $name, $func];
        }
        // native PHP functions
        ...
        if (isset($this->userFunctions[$normalizedName])) {
            ...
            return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION_REFERENCE, 'user', $name, $f, $prototype];
        }
        if (($f = $this->getBuiltinFunction($normalizedName)) && \is_callable($f)) {
            $libName = $f[1];
            $prototype = isset(static::${$libName}) ? static::${$libName} : null;
            return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION_REFERENCE, 'native', $name, $f, $prototype];
        }
        return static::$null;
    }
    /**
    ...
    public function normalizeValue($value)
    {
        ...
        switch ($value[0]) {
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST:
                ...
                if ($value[0] !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST) {
                    return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, $this->compileValue($value)];
                }
     ...
     * @return Number
     */
    protected function opAddNumberNumber(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $left, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $right)
    {
     ...
     * @return Number
     */
    protected function opMulNumberNumber(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $left, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $right)
    {
     ...
     * @return Number
     */
    protected function opSubNumberNumber(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $left, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $right)
    {
     ...
     * @return Number
     */
    protected function opDivNumberNumber(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $left, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $right)
    {
     ...
     * @return Number
     */
    protected function opModNumberNumber(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $left, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $right)
    {
    ...
    protected function opAdd($left, $right)
    {
        if ($strLeft = $this->coerceString($left)) {
            if ($right[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING) {
                $right[1] = '';
            }
            $strLeft[2][] = $right;
            return $strLeft;
        ...
        if ($strRight = $this->coerceString($right)) {
            if ($left[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING) {
                $left[1] = '';
            }
    ...
    protected function opColorColor($op, $left, $right)
    {
        ...
        $out = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COLOR];
        foreach ([1, 2, 3] as $i) {
     ...
     * @return array
     */
    protected function opColorNumber($op, $left, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $right)
    {
        if ($op === '==') {
            return static::$false;
        }
        ...
        return $this->opColorColor($op, $left, [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COLOR, $value, $value, $value]);
    }
    /**
     ...
     * @return array
     */
    protected function opNumberColor($op, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $left, $right)
    {
        if ($op === '==') {
            return static::$false;
        }
        ...
        return $this->opColorColor($op, [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COLOR, $value, $value, $value], $right);
    }
    /**
     ...
     * @return array
     */
    protected function opEqNumberNumber(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $left, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $right)
    {
     ...
     * @return array
     */
    protected function opNeqNumberNumber(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $left, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $right)
    {
        return $this->toBool(!$left->equals($right));
    }
    /**
     ...
     * @return array
     */
    protected function opGteNumberNumber(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $left, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $right)
    {
     ...
     * @return array
     */
    protected function opGtNumberNumber(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $left, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $right)
    {
     ...
     * @return array
     */
    protected function opLteNumberNumber(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $left, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $right)
    {
     ...
     * @return array
     */
    protected function opLtNumberNumber(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $left, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number $right)
    {
        ...
        switch ($value[0]) {
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD:
                if (\is_string($value[1])) {
                    $value[1] = $this->escapeNonPrintableChars($value[1], \true);
                }
                return $value[1];
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COLOR:
                ...
                if (\count($value) === 5) {
                    ...
                    if (!\is_numeric($alpha) || $alpha < 1) {
                        $colorName = \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Colors::RGBaToColorName($r, $g, $b, $alpha);
                        if (!\is_null($colorName)) {
                            return $colorName;
                        }
                        if (\is_numeric($alpha)) {
                            $a = new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number($alpha, '');
                        } else {
                            $a = $alpha;
                        }
                        return 'rgba(' . $r . ', ' . $g . ', ' . $b . ', ' . $a . ')';
                ...
                $colorName = \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Colors::RGBaToColorName($r, $g, $b);
                if (!\is_null($colorName)) {
                    return $colorName;
                }
            ...
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING:
                ...
                if ($value[1]) {
                    ...
                    if ($value[1] === "'" && (\strpos($content, '"') === \false or \strpos($content, "'") !== \false) && \strpbrk($content, '{}\\\'') !== \false) {
                        $value[1] = '"';
            ...
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION:
                ...
                return "{$value[1]}({$args})";
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION_REFERENCE:
            ...
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST:
                ...
                foreach ($items as $item) {
                    if (\is_null($same_string_quote)) {
                        ...
                        if ($item[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING) {
                            $same_string_quote = $item[1];
                            foreach ($items as $ii) {
                                if ($ii[0] !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING) {
                                    $same_string_quote = \false;
                                    break;
                                }
                            }
                    ...
                    if ($item[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NULL) {
                        continue;
                    }
                    if ($same_string_quote === '"' && $item[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING && $item[1]) {
            ...
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_INTERPOLATED:
            ...
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_INTERPOLATE:
                ...
                switch ($reduced[0]) {
                    case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST:
                        ...
                        if ($reduced[0] !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST) {
                            break;
                        }
                        ...
                        foreach ($items as $item) {
                            ...
                            $temp = $this->compileValue([\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, $item]);
                            if ($temp[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING) {
                                $filtered[] = $this->compileStringContent($temp);
                            } elseif ($temp[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD) {
                                $filtered[] = $temp[1];
                            } else {
                                $filtered[] = $this->compileValue($temp);
                            }
                        }
                        $reduced = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, \implode("{$delim}", $filtered)];
                        break;
                    case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING:
                        $reduced = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', [$this->compileStringContent($reduced)]];
                        break;
                    case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NULL:
                        $reduced = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, ''];
                }
    ...
    protected function extractInterpolation($list)
    {
        ...
        foreach ($items as $i => $item) {
            if ($item[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_INTERPOLATE) {
                $before = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, $list[1], \array_slice($items, 0, $i)];
                $after = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, $list[1], \array_slice($items, $i + 1)];
                return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_INTERPOLATED, $item, $before, $after];
            }
        }
        return $list;
    }
     ...
     * @return array
     */
    protected function multiplySelectors(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Compiler\Environment $env, $selfParent = null)
    {
        $envs = $this->compactEnv($env);
        $selectors = [];
        $parentSelectors = [[]];
     ...
     * @return array
     */
    protected function multiplyMedia(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Compiler\Environment $env = null, $childQueries = null)
    {
        if (!isset($env) || !empty($env->block->type) && $env->block->type !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MEDIA) {
            return $childQueries;
        }
        ...
        $parentQueries = isset($env->block->queryList) ? $env->block->queryList : [[[\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MEDIA_VALUE, $env->block->value]]];
        ...
        if (\is_null($childQueries)) {
            ...
            foreach ($parentQueries as $parentQuery) {
                foreach ($originalQueries as $childQuery) {
                    $childQueries[] = \array_merge($parentQuery, [[\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MEDIA_TYPE, [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, 'all']]], $childQuery);
                }
            }
        }
        return $this->multiplyMedia($env->parent, $childQueries);
     ...
     */
    protected function compactEnv(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Compiler\Environment $env)
    {
     ...
     */
    protected function pushEnv(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Block $block = null)
    {
        $env = new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Compiler\Environment();
        $env->parent = $this->env;
     ...
     * @return void
     */
    protected function set($name, $value, $shadow = \false, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Compiler\Environment $env = null, $valueUnreduced = null)
    {
     ...
     * @return void
     */
    protected function setExisting($name, $value, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Compiler\Environment $env, $valueUnreduced = null)
    {
        $storeEnv = $env;
     ...
     * @return void
     */
    protected function setRaw($name, $value, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Compiler\Environment $env, $valueUnreduced = null)
    {
        $env->store[$name] = $value;
        if ($valueUnreduced) {
            $env->storeUnreduced[$name] = $valueUnreduced;
     ...
     * @return mixed|null
     */
    public function get($name, $shouldThrow = \true, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Compiler\Environment $env = null, $unreduced = \false)
    {
        $normalizedName = $this->normalizeName($name);
        $specialContentKey = static::$namespaces['special'] . 'content';
        if (!isset($env)) {
     ...
     * @return boolean
     */
    protected function has($name, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Compiler\Environment $env = null)
    {
    ...
    {
        switch ($style) {
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\OutputStyle::EXPANDED:
                $this->formatter = \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\Expanded::class;
                break;
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\OutputStyle::COMPRESSED:
                $this->formatter = \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\Compressed::class;
                break;
            default:
    ...
    {
        if (!\in_array($formatterName, [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\Expanded::class, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\Compressed::class], \true)) {
     ...
     * @return void
     */
    protected function importFile($path, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\OutputBlock $out)
    {
    ...
    public function error($msg, ...$args)
    {
        ...
        return new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Exception\CompilerException($msg);
    }
    /**
        ...
        if ($this->callStack) {
            foreach (\array_reverse($this->callStack) as $call) {
                if ($all || isset($call['n']) && $call['n']) {
                    $msg = '#' . $ncall++ . ' ' . $call['n'] . ' ';
                    $msg .= isset($this->sourceNames[$call[\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Parser::SOURCE_INDEX]]) ? $this->getPrettyPath($this->sourceNames[$call[\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Parser::SOURCE_INDEX]]) : '(unknown file)';
                    $msg .= ' on line ' . $call[\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Parser::SOURCE_LINE];
                    $callStackMsg[] = $msg;
    ...
    protected function callScssFunction($func, $argValues)
    {
        ...
        $tmp = new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Formatter\OutputBlock();
        $tmp->lines = [];
        $tmp->children = [];
    ...
    {
        return \method_exists(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Compiler::class, self::normalizeNativeFunctionName($name));
    }
    /**
    ...
    protected function sortNativeFunctionArgs($functionName, $prototypes, $args)
    {
        ...
        if (\in_array($functionName, ['libRgb', 'libRgba', 'libHsl', 'libHsla'])) {
            ...
            foreach ($args as $k => $arg) {
                if ($arg[1][0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST && \count($arg[1][2]) === 3) {
                    $last = \end($arg[1][2]);
                    if ($last[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_EXPRESSION && $last[1] === '/') {
                        \array_pop($arg[1][2]);
                        $arg[1][2][] = $last[2];
                        $arg[1][2][] = $last[3];
                        $args[$k] = $arg;
        ...
        foreach ($prototypes as $prototype) {
            ...
            foreach ($prototype as $i => $p) {
                ...
                if (\count($p)) {
                    ...
                    if ($p === 'null') {
                        ...
                        $default = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, 'null'];
                    } else {
                        if (\is_null($parser)) {
                            $parser = $this->parserFactory(__METHOD__);
                        }
            ...
            try {
                ...
                foreach ($prototype as $i => $p) {
                    ...
                    if ($finalArgs[$i] === [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, 'null']) {
                        $finalArgs[$i] = null;
                    }
                }
                // should we break if this prototype seems fulfilled?
            } catch (\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Exception\CompilerException $e) {
        ...
        if ($exceptionMessage && !$prototypeHasMatch) {
            if (\in_array($functionName, ['libRgb', 'libRgba', 'libHsl', 'libHsla'])) {
                ...
                foreach ($args as $arg) {
                    if ($arg[1][0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION_CALL && \in_array($arg[1][1], ['var'])) {
                        return null;
                    }
                }
            }
    ...
    protected function applyArguments($argDef, $argValues, $storeInEnv = \true, $reduce = \true)
    {
        ...
        foreach ((array) $argValues as $arg) {
            if (!empty($arg[0])) {
                ...
                if ($val[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST) {
                ...
                } elseif ($val[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MAP) {
        ...
        foreach ($args as $arg) {
            ...
            if ($isVariable) {
                ...
                $val = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, \is_null($splatSeparator) ? ',' : $splatSeparator, [], $isVariable];
    ...
    protected function coerceValue($value)
    {
        ...
        if (\is_numeric($value)) {
            return new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number($value, '');
        }
        if ($value === '') {
            return static::$emptyString;
        }
        $value = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, $value];
    ...
    {
        if ($item[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MAP) {
            return $item;
        }
    ...
    {
        if (isset($item) && $item[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST) {
        ...
        if (isset($item) && $item[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MAP) {
            $keys = $item[1];
            $values = $item[2];
            $list = [];
            for ($i = 0, $s = \count($keys); $i < $s; $i++) {
                ...
                switch ($key[0]) {
                    ...
                    case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_NULL:
                        break;
                    default:
                        $key = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, $this->compileStringContent($this->coerceString($key))];
                        break;
                }
                $list[] = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, '', [$key, $value]];
            }
            return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, ',', $list];
        }
        return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, $delim, !isset($item) ? [] : [$item]];
    }
    /**
    ...
    protected function coerceColor($value, $inRGBFunction = \false)
    {
        switch ($value[0]) {
            ...
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST:
                if ($inRGBFunction) {
                    if (\count($value[2]) == 3 || \count($value[2]) == 4) {
                        $color = $value[2];
                        \array_unshift($color, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COLOR);
            ...
            case \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD:
                    ...
                    if (\in_array($nofValues, [3, 4, 6, 8])) {
                        ...
                        if ($nbChannels === 4) {
                            if ($color[3] === 255) {
                                ...
                                $color[3] = \round($color[3] / 255, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number::PRECISION);
                            }
                        }
                        \array_unshift($color, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COLOR);
                        return $color;
                ...
                if ($rgba = \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Colors::colorNameToRGBa($name)) {
                    return isset($rgba[3]) ? [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COLOR, $rgba[0], $rgba[1], $rgba[2], $rgba[3]] : [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COLOR, $rgba[0], $rgba[1], $rgba[2]];
                }
                return null;
        }
        return null;
    ...
    protected function compileColorPartValue($value, $min, $max, $isInt = \true)
    {
        if (!\is_numeric($value)) {
            if (\is_array($value)) {
                ...
                if ($reduced instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number) {
                    $value = $reduced;
                }
            }
            if ($value instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number) {
                if ($value->unitless()) {
    ...
    {
        if ($value[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING) {
            return $value;
        }
        return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', [$this->compileValue($value)]];
    }
    /**
    ...
    public function assertString($value, $varName = null)
    {
        ...
        if ($value[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION) {
            $value = $this->coerceString($value);
        }
        if (!\in_array($value[0], [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD])) {
            $value = $this->compileValue($value);
    ...
    public function assertMap($value)
    {
        ...
        if ($value[0] !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MAP) {
    ...
    public function assertInteger($value, $varName = null)
    {
        ...
        if (\round($value - \intval($value), \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number::PRECISION) > 0) {
            $var_display = $varName ? " \${$varName}:" : '';
    ...
    public function toHSL($red, $green, $blue)
    {
        ...
        return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_HSL, \fmod($h, 360), $s * 100, $l / 5.1];
    }
    /**
     * Hue to RGB helper
     *
    ...
    public function toRGB($hue, $saturation, $lightness)
    {
        ...
        $out = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COLOR, $r, $g, $b];
        return $out;
    }
    // Built in functions
    ...
    protected function libCall($args, $kwargs)
    {
        ...
        if (\in_array($functionReference[0], [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD])) {
        ...
        if (!\in_array($functionReference[0], [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION_REFERENCE, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION])) {
        ...
        return $this->reduce([\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION_CALL, $functionReference, $callArgs]);
    }
    ...
    protected function libGetFunction($args)
    {
        ...
        if ($isCss) {
            return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION, $name, [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, ',', []]];
        }
    ...
    protected function libIndex($args)
    {
        ...
        if ($list[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MAP || $list[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING || $list[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD || $list[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_INTERPOLATE) {
            $list = $this->coerceList($list, ' ');
        }
        if ($list[0] !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST) {
            return static::$null;
        }
        ...
        if ($value instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number) {
            ...
            foreach ($list[2] as $item) {
                ...
                if ($itemValue instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number && $value->equals($itemValue)) {
                    return new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number($key, '');
                }
            }
            return static::$null;
        }
        ...
        switch (\count($args)) {
            case 1:
                if (!($color = $this->coerceColor($args[0], \true))) {
                    $color = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $args[0], ')']];
                }
                break;
            case 3:
                $color = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COLOR, $args[0], $args[1], $args[2]];
                if (!($color = $this->coerceColor($color))) {
                    $color = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $args[0], ', ', $args[1], ', ', $args[2], ')']];
                }
                return $color;
            case 2:
                if ($color = $this->coerceColor($args[0], \true)) {
                    ...
                    if (\is_numeric($alpha)) {
                        ...
                        $color = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $color[1], ', ', $color[2], ', ', $color[3], ', ', $alpha, ')']];
                    }
                } else {
                    $color = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $args[0], ')']];
                }
            ...
            case 4:
            default:
                $color = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COLOR, $args[0], $args[1], $args[2], $args[3]];
                if (!($color = $this->coerceColor($color))) {
                    $color = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $args[0], ', ', $args[1], ', ', $args[2], ', ', $args[3], ')']];
                }
                break;
        }
        return $color;
    ...
    protected function libIeHexStr($args)
    {
        ...
        return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', [\sprintf('#%02X%02X%02X%02X', $color[4], $color[1], $color[2], $color[3])]];
    }
    ...
    protected function libMix($args)
    {
        ...
        $w2 = 1.0 - $w1;
        $new = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_COLOR, $w1 * $first[1] + $w2 * $second[1], $w1 * $first[2] + $w2 * $second[2], $w1 * $first[3] + $w2 * $second[3]];
    ...
    protected function libHsl($args, $kwargs, $funcName = 'hsl')
    {
        $args_to_check = $args;
        if (\count($args) == 1) {
            if ($args[0][0] !== \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST || \count($args[0][2]) < 3 || \count($args[0][2]) > 4) {
                return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $args[0], ')']];
            }
            $args = $args[0][2];
            $args_to_check = $kwargs['channels'][2];
        }
        foreach ($kwargs as $k => $arg) {
            if (\in_array($arg[0], [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION_CALL]) && \in_array($arg[1], ['min', 'max'])) {
                return null;
            }
        }
        foreach ($args_to_check as $k => $arg) {
            ...
            if ($k >= 2 && \count($args) === 4 && \in_array($arg[0], [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION_CALL, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_FUNCTION]) && \in_array($arg[1], ['calc', 'env'])) {
                return null;
            }
        }
        ...
        if (\count($args) === 4) {
            ...
            if (!$hue instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number || !$saturation instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number || !$lightness instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number || !\is_numeric($alpha)) {
                return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $args[0], ', ', $args[1], ', ', $args[2], ', ', $args[3], ')']];
            }
        } else {
            if (!$hue instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number || !$saturation instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number || !$lightness instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number) {
                return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', [$funcName . '(', $args[0], ', ', $args[1], ', ', $args[2], ')']];
            }
        }
    ...
    protected function libHue($args)
    {
        $color = $this->assertColor($args[0]);
        $hsl = $this->toHSL($color[1], $color[2], $color[3]);
        return new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number($hsl[1], 'deg');
    }
    ...
    protected function libSaturation($args)
    {
        $color = $this->assertColor($args[0]);
        $hsl = $this->toHSL($color[1], $color[2], $color[3]);
        return new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number($hsl[2], '%');
    }
    ...
    protected function libLightness($args)
    {
        $color = $this->assertColor($args[0]);
        $hsl = $this->toHSL($color[1], $color[2], $color[3]);
        return new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number($hsl[3], '%');
    }
    ...
    {
        $color = $this->assertColor($args[0]);
        $amount = \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Util::checkRange('amount', new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Base\Range(0, 100), $args[1], '%');
    ...
    protected function libInvert($args)
    {
        ...
        if ($weight < 1) {
            return $this->libMix([$inverted, $color, new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number($weight, '')]);
        }
        return $inverted;
    }
    ...
    {
        $str = $args[0];
        if ($str[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING) {
            $str[1] = '';
        }
        return $str;
    }
    ...
    {
        $value = $args[0];
        if ($value[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING && !empty($value[1])) {
            $value[1] = '"';
            return $value;
        }
        return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '"', [$value]];
    }
    ...
    protected function libPercentage($args)
    {
        ...
        return new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number($num->getDimension() * 100, '%');
    }
    protected static $libRound = ['number'];
    protected function libRound($args)
    {
        $num = $this->assertNumber($args[0], 'number');
        return new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number(\round($num->getDimension()), $num->getNumeratorUnits(), $num->getDenominatorUnits());
    }
    protected static $libFloor = ['number'];
    protected function libFloor($args)
    {
        $num = $this->assertNumber($args[0], 'number');
        return new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number(\floor($num->getDimension()), $num->getNumeratorUnits(), $num->getDenominatorUnits());
    }
    protected static $libCeil = ['number'];
    protected function libCeil($args)
    {
        $num = $this->assertNumber($args[0], 'number');
        return new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number(\ceil($num->getDimension()), $num->getNumeratorUnits(), $num->getDenominatorUnits());
    }
    protected static $libAbs = ['number'];
    protected function libAbs($args)
    {
        $num = $this->assertNumber($args[0], 'number');
        return new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number(\abs($num->getDimension()), $num->getNumeratorUnits(), $num->getDenominatorUnits());
    }
    ...
    protected function libListSeparator($args)
    {
        ...
        if (!\in_array($args[0][0], [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MAP])) {
            return 'space';
        }
    ...
    protected function libMapKeys($args)
    {
        $map = $this->assertMap($args[0]);
        $keys = $map[1];
        return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, ',', $keys];
    }
    ...
    protected function libMapValues($args)
    {
        $map = $this->assertMap($args[0]);
        $values = $map[2];
        return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, ',', $values];
    }
    ...
    protected function libKeywords($args)
    {
        ...
        foreach ($args[0][2] as $name => $arg) {
            $keys[] = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, $name];
            $values[] = $arg;
        }
        return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_MAP, $keys, $values];
    }
    ...
    protected function libJoin($args)
    {
        ...
        if ($bracketed === static::$true) {
            ...
            $bracketed = \false;
        } elseif ($bracketed === [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, 'auto']) {
            $bracketed = 'auto';
        ...
        $res = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, $sep, \array_merge($list1[2], $list2[2])];
    ...
    protected function libAppend($args)
    {
        ...
        $list1 = $this->coerceList($list1, ' ', \true);
        $sep = $this->listSeparatorForJoin($list1, $sep);
        $res = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, $sep, \array_merge($list1[2], [$value])];
        if (isset($list1['enclosing'])) {
    ...
    protected function libZip($args)
    {
        ...
        $result = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, ',', $lists];
        if (!\is_null($firstList)) {
            foreach ($firstList[2] as $key => $item) {
                $list = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, '', [$item]];
                foreach ($args as $arg) {
                    if (isset($arg[2][$key])) {
                        $list[2][] = $arg[2][$key];
                    } else {
    ...
    {
        $num = $args[0];
        if ($num instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number) {
            return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '"', [$num->unitStr()]];
        }
        return '';
    }
    ...
    {
        $value = $args[0];
        return $value instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number && $value->unitless();
    }
    protected static $libComparable = [['number1', 'number2'], ['number-1', 'number-2']];
    protected function libComparable($args)
    {
        list($number1, $number2) = $args;
        if (!$number1 instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number || !$number2 instanceof \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number) {
    ...
    protected function libStrIndex($args)
    {
        ...
        if (!\strlen($substringContent)) {
            ...
            $result = \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Util::mbStrpos($stringContent, $substringContent);
        }
        return $result === \false ? static::$null : new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number($result + 1, '');
    }
    ...
    protected function libStrInsert($args)
    {
        ...
        if ($index < 0) {
            $index = \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Util::mbStrlen($stringContent) + 1 + $index;
        }
        $string[2] = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Util::mbSubstr($stringContent, 0, $index), $insertContent, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Util::mbSubstr($stringContent, $index)];
        return $string;
    }
    ...
    protected function libStrLength($args)
    {
        $string = $this->assertString($args[0], 'string');
        $stringContent = $this->compileStringContent($string);
        return new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number(\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Util::mbStrlen($stringContent), '');
    }
    ...
    {
        $mblength = \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Util::mbStrlen($stringContent);
        if ($mblength === \strlen($stringContent)) {
            return $filter($stringContent);
        }
        $filteredString = "";
        for ($i = 0; $i < $mblength; $i++) {
            $char = \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Util::mbSubstr($stringContent, $i, 1);
            if (\strlen($char) > 1) {
                $filteredString .= $char;
            } else {
    ...
    protected function libCounter($args)
    {
        ...
        return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', ['counter(' . \implode(',', $list) . ')']];
    }
    ...
    protected function libRandom($args)
    {
        if (isset($args[0]) & $args[0] !== static::$null) {
            ...
            if (\round($n - \intval($n), \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number::PRECISION) > 0) {
                throw $this->error("Expected \$limit to be an integer but got {$n} for `random`");
            }
            return new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number(\mt_rand(1, \intval($n)), '');
        }
        $max = \mt_getrandmax();
        return new \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Node\Number(\mt_rand(0, $max - 1) / $max, '');
    }
    protected function libUniqueId()
    {
        ...
        $id += \mt_rand(0, 10) + 1;
        return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', ['u' . \str_pad(\base_convert($id, 10, 36), 8, '0', \STR_PAD_LEFT)]];
    }
    protected function inspectFormatValue($value, $force_enclosing_display = \false)
    {
        if ($value === static::$null) {
            $value = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD, 'null'];
        }
        $stringValue = [$value];
        if ($value[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST) {
            if (\end($value[2]) === static::$null) {
                \array_pop($value[2]);
                $value[2][] = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', ['']];
        ...
        return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', $stringValue];
    }
    ...
    {
        if ($arg[0] === \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST && $maxDepth > 0) {
        ...
        if (!\in_array($arg[0], [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, \WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_KEYWORD])) {
            return \false;
        }
        return \true;
    }
    ...
    protected function libSimpleSelectors($args)
    {
        ...
        $listParts = [];
        foreach ($part as $p) {
            $listParts[] = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '', [$p]];
        }
        return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, ',', $listParts];
    }
    ...
    protected function libScssphpGlob($args)
    {
        ...
        foreach ($matches as $match) {
            ...
            $listParts[] = [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_STRING, '"', [$match]];
        }
        return [\WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Type::T_LIST, ',', $listParts];
    }
}
