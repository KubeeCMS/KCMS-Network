 ...
 */
namespace WP_Ultimo\Dependencies\ScssPhp\ScssPhp\SourceSpan;
...
final class SourceFile
{
    ...
    private function isNearCacheLine(int $position) : bool
    {
        ...
        if ($this->cachedLine >= \count($this->lineStarts) - 1 || $position < $this->lineStarts[$this->cachedLine + 1]) {
            return \true;
        }
        if ($this->cachedLine >= \count($this->lineStarts) - 2 || $position < $this->lineStarts[$this->cachedLine + 2]) {
            ++$this->cachedLine;
            return \true;
        }
        return \false;
