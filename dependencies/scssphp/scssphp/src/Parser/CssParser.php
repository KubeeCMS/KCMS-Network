...
final class CssParser extends ScssParser
{
     ...
     */
    private const CSS_ALLOWED_FUNCTIONS = ['rgb' => \true, 'rgba' => \true, 'hsl' => \true, 'hsla' => \true, 'grayscale' => \true, 'invert' => \true, 'alpha' => \true, 'opacity' => \true, 'saturate' => \true];
    ...
    private function cssImportRule(int $start) : ImportRule
    {
        ...
        return new ImportRule([new StaticImport(new Interpolation([$url], $urlSpan), $this->scanner->spanFrom($start), $supports, $media)], $this->scanner->spanFrom($start));
    }
    protected function identifierLike() : Expression
    {
        ...
        if (!$this->scanner->scanChar(')')) {
            do {
                $this->whitespace();
                $arguments[] = $this->expression(null, \true);
                $this->whitespace();
