...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Sass\Argument;
...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Sass\Expression\BinaryOperator;
...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Sass\Expression\UnaryOperator;
...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Sass\Import\StaticImport;
...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Sass\Statement\ElseClause;
...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Sass\Statement\IfClause;
...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Sass\SupportsCondition\SupportsAnything;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Sass\SupportsCondition\SupportsDeclaration;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Sass\SupportsCondition\SupportsFunction;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Sass\SupportsCondition\SupportsInterpolation;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Sass\SupportsCondition\SupportsNegation;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Sass\SupportsCondition\SupportsOperation;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Colors;
...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Util\Character;
...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Value\SpanColorFormat;
/**
 * @internal
 */
abstract class StylesheetParser extends Parser
{
    ...
    protected function expression(?callable $until = null, bool $singleEquals = \false, bool $bracketList = \false) : Expression
    {
        ...
        $addOperator = function (string $operator) use(&$allowSlash, &$operators, &$operands, &$singleExpression, $resolveOneOperation) : void {
    ...
    private function calculationSum() : Expression
    {
        ...
        while (\true) {
            ...
            if ($next === '+' || $next === '-') {
                ...
                $this->scanner->readChar();
                $this->whitespace();
                $sum = new BinaryOperationExpression($next === '+' ? BinaryOperator::PLUS : BinaryOperator::MINUS, $sum, $this->calculationProduct());
            } else {
                return $sum;
            }
        }
    ...
    private function calculationProduct() : Expression
    {
        ...
        while (\true) {
            ...
            if ($next === '*' || $next === '/') {
                $this->scanner->readChar();
                $this->whitespace();
                $product = new BinaryOperationExpression($next === '*' ? BinaryOperator::TIMES : BinaryOperator::DIVIDED_BY, $product, $this->calculationValue());
            } else {
                return $product;
            }
        }
    ...
    private function containsCalculationInterpolation() : bool
    {
        ...
        while (!$this->scanner->isDone()) {
            ...
            switch ($next) {
                ...
                case '[':
                    \assert($next !== null);
                    // https://github.com/phpstan/phpstan/issues/5678
     ...
     */
    private function interpolatedDeclarationValue(bool $allowEmpty = \false, bool $allowSemicolon = \false, bool $allowColon = \true) : Interpolation
    {
        $start = $this->scanner->getPosition();
        $buffer = new InterpolationBuffer();
        $brackets = [];
     ...
     */
    protected abstract function isIndented() : bool;
    /**
     ...
     * {@see scanElse}.
     */
    protected abstract function getCurrentIndentation() : int;
    /**
     * Parses and returns a selector used in a style rule.
     */
    protected abstract function styleRuleSelector() : Interpolation;
    /**
     ...
     * @throws FormatException
     */
    protected abstract function expectStatementSeparator(?string $name = null) : void;
    /**
     * Whether the scanner is positioned at the end of a statement.
     */
    protected abstract function atEndOfStatement() : bool;
    /**
     * Whether the scanner is positioned before a block of children that can be
     * parsed with {@see children}.
     */
    protected abstract function lookingAtChildren() : bool;
    /**
     ...
     */
    protected abstract function scanElse(int $ifIndentation) : bool;
    /**
     ...
     * @return Statement[]
     */
    protected abstract function children(callable $child) : array;
    /**
     ...
     * @return Statement[]
     */
    protected abstract function statements(callable $statement) : array;
}
