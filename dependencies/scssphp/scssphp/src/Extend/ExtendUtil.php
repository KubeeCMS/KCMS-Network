 ...
 */
namespace WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Extend;
...
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Selector\QualifiedName;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Selector\SelectorList;
use WP_Ultimo\Dependencies\ScssPhp\ScssPhp\Ast\Selector\SimpleSelector;
...
final class ExtendUtil
{
     ...
     */
    private const SUBSELECTOR_PSEUDOS = ['is', 'matches', 'any', 'nth-child', 'nth-last-child'];
    /**
    ...
    {
        if ((\count($components1) === 0 || !\is_string($components1[\count($components1) - 1])) && (\count($components2) === 0 || !\is_string($components2[\count($components2) - 1]))) {
            return $result;
        }
        /**
        ...
        if ($combinator1 !== null && $combinator2 !== null) {
            ...
            if ($combinator1 === Combinator::FOLLOWING_SIBLING && $combinator2 === Combinator::FOLLOWING_SIBLING) {
                if ($compound1->isSuperselector($compound2)) {
                    ...
                    $choices = [[$compound1, Combinator::FOLLOWING_SIBLING, $compound2, Combinator::FOLLOWING_SIBLING], [$compound2, Combinator::FOLLOWING_SIBLING, $compound1, Combinator::FOLLOWING_SIBLING]];
                ...
                if ($followingSiblingSelector->isSuperselector($nextSiblingSelector)) {
                    ...
                    $unified = self::unifyCompound($compound1->getComponents(), $compound2->getComponents());
                    $choices = [[$followingSiblingSelector, Combinator::FOLLOWING_SIBLING, $nextSiblingSelector, Combinator::NEXT_SIBLING]];
                    if ($unified !== null) {
    ...
    private static function simpleIsSuperselectorOfCompound(SimpleSelector $simple, CompoundSelector $compound) : bool
    {
        foreach ($compound->getComponents() as $theirSimple) {
            ...
            if (!\in_array($theirSimple->getNormalizedName(), self::SUBSELECTOR_PSEUDOS, \true)) {
                return \false;
            }
